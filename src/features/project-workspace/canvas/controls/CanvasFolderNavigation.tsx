      ...
      >
        ...
        <span className="hidden shrink-0 items-center gap-0.5 group-hover:flex">
          ...
          {canEnter ? (
            <button
              type="button"
              className="rounded-md p-1 text-[var(--glass-text-tertiary)] hover:bg-white hover:text-[var(--glass-text-primary)]"
              title={enterTitle}
              onClick={(event) => {
                event.stopPropagation()
  ...
  if (!open) {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <button
          type="button"
          className={`${PANEL_BUTTON_CLASS} gap-1.5 px-3.5 text-[12px] font-medium`}
          onClick={() => setOpen(true)}
        >
          <AppIcon name="folder" className="h-3.5 w-3.5 text-amber-500" />
          {t('directory')}
        </button>
      </div>
    )
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 pb-1.5">
        ...
        <AppIcon name="folder" className="ml-1 h-3.5 w-3.5 text-amber-500" />
