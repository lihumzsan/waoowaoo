...
export default function EditScriptPreviewDetail({
  ...
  const modal = (
    ...
    >
      <div
        className="mx-auto flex max-h-[calc(100vh-3rem)] w-full max-w-6xl flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_24px_90px_rgba(15,23,42,0.28)]"
      ...
      <DetailSection title={t('sections.editScriptPreview')}>
        <div className="grid gap-4 lg:grid-cols-[minmax(280px,0.9fr)_minmax(0,1.1fr)]">
          <div className="space-y-3">
              ...
              <div className="flex aspect-video items-center justify-center bg-black">
                {activeShot?.videoUrl ? (
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-slate-400">
                    ...
                    <span className="text-xs font-semibold">{t('empty.noPreviewMedia')}</span>
                  </div>
                )}
              </div>
            ...
            <button
              type="button"
              className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-900 disabled:cursor-not-allowed disabled:bg-slate-400"
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-md bg-white p-3 text-sm">
                <p className="text-xs font-semibold uppercase text-[var(--glass-text-tertiary)]">{t('fields.duration')}</p>
                <p className="mt-1 font-semibold text-[var(--glass-text-primary)]">{formatSeconds(totalDurationSec)}</p>
              </div>
              <div className="rounded-md bg-white p-3 text-sm">
                <p className="text-xs font-semibold uppercase text-[var(--glass-text-tertiary)]">{t('fields.shotCount')}</p>
                <p className="mt-1 font-semibold text-[var(--glass-text-primary)]">{shots.length}</p>
              </div>
              <div className="rounded-md bg-white p-3 text-sm">
                <p className="text-xs font-semibold uppercase text-[var(--glass-text-tertiary)]">{t('fields.currentShot')}</p>
            ...
            {activeShot ? (
              <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full border border-slate-200 px-2.5 py-1 text-xs font-semibold text-[var(--glass-text-secondary)]">
