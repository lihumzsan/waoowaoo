...
export async function handleCreativeResourceVideoMergeTask(
  ...
  const payload = parseCreativeResourceVideoMergeTaskPayload(data.payload ?? {})
  if (payload.resource.resourceId !== data.targetId) {
    throw new Error(`CREATIVE_RESOURCE_VIDEO_MERGE_TARGET_MISMATCH:${data.taskId}`)
  }
  ...
  const resolveInputStorageKey = (input: (typeof payload.resource.inputs)[number]): string => {
    ...
    if (
      ...
      || (resource.projectId && resource.projectId !== data.projectId)
  ...
  if (bgmInput && bgmVolume === null) {
    throw new Error(`CREATIVE_RESOURCE_VIDEO_MERGE_BGM_VOLUME_INVALID:${data.taskId}`)
  }

  const workspaceDir = await mkdtemp(path.join(tmpdir(), `waoowaoo-resource-merge-${randomUUID()}-`))
  try {
    await reportTaskProgress(context, 10, { stage: 'creative_resource_video_merge_prepare' })
    ...
    await reportTaskProgress(context, 65, { stage: 'creative_resource_video_merge_compose' })
    ...
    await reportTaskProgress(context, 92, { stage: 'creative_resource_video_merge_persist' })
    await assertTaskActive(context, 'persist_creative_resource_video_merge')
