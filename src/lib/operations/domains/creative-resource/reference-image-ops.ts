...
const importWebReferenceImageInputSchema = z
  .object({
    imageUrl: httpUrlSchema.describe(
      'Exact http(s) image URL copied verbatim from one web_search image result. Never a page URL, a guessed URL, or a URL from anywhere else.',
    ),
    sourceWebsiteUrl: httpUrlSchema.describe(
      'Exact http(s) page URL the same image result was found on. Provenance is mandatory and is stored with the imported material.',
    ),
    name: z
      .string()
      .trim()
      ...
      .max(200)
      .describe(
        'Short display name for this reference material, in the conversation language.',
      ),
    caption: z
      .string()
      .trim()
      ...
      .nullable()
      .describe(
        'Caption returned with that image result, or null when it had none. Never substitute your own description.',
      ),
  })
  .strict()

const importWebReferenceImageOutputSchema =
  refineTaskSubmitOperationOutputSchema(
    taskSubmitOperationOutputSchemaBase
      .extend({
...
function assertHttpUrl(
  value: string,
  field: 'imageUrl' | 'sourceWebsiteUrl',
): string {
  ...
  return {
    import_web_reference_image: defineOperation({
      ...
      assistantWriteAuthority: {
        ...
        contractRevision: 'import_web_reference_image:v1',
      ...
      execute: async (ctx, input) => {
        ...
        const sourceWebsiteUrl = assertHttpUrl(
        ...
        return importWebReferenceImageOutputSchema.parse({
          ...result,
          resourceId,
        })
      },
