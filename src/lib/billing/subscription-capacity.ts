 ...
 *
 * A number of credits means nothing on its own — nobody knows whether 5,450 is
 * a lot. These reference figures answer the question people actually have when
 * comparing plans: how many clips, how many pictures.
 *
 * Each figure assumes the whole balance goes to that one kind of work; they are
 * alternatives, not a sum. The pricing page's footnote has to say so, because
 * two numbers side by side otherwise read as things you get both of.
 *
...
/** A representative clip: Seedance 2.0, 720p, five seconds. */
 ...
 *
 * Rounded down, because "about 175 images" must never turn out to be 174 when
 * the user counts.
 */
