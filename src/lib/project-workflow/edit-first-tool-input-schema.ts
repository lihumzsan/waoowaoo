...
export const EDIT_FIRST_INGEST_SCRIPT_TOOL_INPUT_SCHEMA: ProjectAgentToolInputSchema = createToolInputSchema({
  sourceKind: {
    ...
    description: 'Classify the user input. Use paste only for a complete filmable script provided by the user; it can generate the episode plan directly. Use prompt_generated_outline only for a post-intake normalizedBrief; it expands a full script and then waits for user script confirmation before episode planning. Titles, theme directions, one-line ideas, short loglines, and briefs missing basic script conditions must use request_script_intake_choice first.',
  },
