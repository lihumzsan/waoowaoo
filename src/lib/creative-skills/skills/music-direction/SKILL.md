...
本 Skill 负责剧情配乐判断、cue 窗口、Composition Plan 与混音意图。模型选择、能力校验、Operation Plan、Task、durable provider invocation、日志错误码、Resource 终态和最终合成全部由系统既有统一链路负责；不得调用 Provider 的自动 plan/video-to-music 能力另造第二个配乐决策者。
...
- 只从系统注入的 `productionCapabilities.sound` 读取能力。能力为空时停止提交，不得猜测模型、时长或格式。
...
音效自检：是否只有一个可识别声场或事件？是否写明空间与距离而非抽象气氛词？是否在 `productionCapabilities.sound` 的时长和 `mp3` 能力内？是否显式标记 `audioKind` 和 `schemaId`，且没有任何 references？
