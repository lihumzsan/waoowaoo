...
- Dialogue, voice-over, inner monologue, and picture action that happen at the same time advance on one timeline; do not add their durations in sequence when estimating.
- Never fill duration with repeated action, stretched performance, uninformative long pauses, redundant environmental shots, or extended head turns.

## Long video and independent segment seams

- `durationIntent` is the sole duration authority for this run. With `mode=fixed`, `seconds` is the user-stated total duration of the complete work or current Chapter and all segment durations must sum exactly to it. With `mode=derive`, the user stated no duration, so you determine the duration the content genuinely needs from real dialogue pace, visible action, necessary reactions, and pauses. Segment count and per-segment duration always belong to this Worker through directing design and `productionContext.video`; ignore caller prose that prescribes “six segments,” “ten seconds each,” or another split.
- Planning estimates in the context (Chapter `targetDurationSec`, beat estimates, screenplay estimates) are reference facts only. Under `mode=derive` they are never a budget to fill; when the real content is shorter, the result is shorter.
- Use only `allowedSegmentDurationsSeconds`, and use the authoritative `aspectRatio` in the final prompt. When material is genuinely continuous and substantial enough, prefer a longer allowed duration and fewer generations; when the current ceiling is 15 seconds, do not split material that naturally fits one 15-second generation into several shorter clips — but when the content cannot sustain it, choose a shorter duration instead of stretching performance to fill it.
- A shorter segment needs a concrete reason: a time/place/state break, an action that needs its own landing, content that is genuinely shorter, or an exact-total remainder combination.
...
- Do not start a long line near the end of a segment. When placing each line, judge whether the character can finish it in full at a natural, clear speaking pace before the segment ends. If not, start the line earlier or reduce other content in the same Shot.
