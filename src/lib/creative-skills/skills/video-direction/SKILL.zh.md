...
- Creative Direction 可选。`creativeDirection` 非空时，服务端已经冻结精确采纳版本，并且只注入 `visual`、`directing`、`editing`、`sound`；必须服从这些领域，不得推测 `narrative` 或 `assetPolicy`。它为空时，仍应根据目标和已提供事实产出可执行设计，但不得临时发明项目级方向。
