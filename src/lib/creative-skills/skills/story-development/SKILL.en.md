...
- For `outputKind=chapter_plan`, use the complete, exactly sourced screenplay as the only source text for Chapter boundaries. A Story Canon, continuity analysis, or Style Story Canon is optional context only when actually supplied; their absence is never a failure condition.
