...
For `outputKind=screenplay`, Creative Direction is optional. When `creativeDirection` is non-null, consume only its server-injected `narrative` policy; do not infer visual, directing, editing, sound, or asset rules. When it is null, write from the supplied user request and story facts without inventing a project-wide direction.

## Chapter planning

- For `outputKind=chapter_plan`, use the complete, exactly sourced screenplay as the only source text for Chapter boundaries. A Story Canon or continuity analysis is optional context only when actually supplied; their absence is never a failure condition. Chapter planning receives no Creative Direction.
