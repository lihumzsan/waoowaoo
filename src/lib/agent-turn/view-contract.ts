...
export type AgentSessionFollowUpBatchStatus = 'pending' | 'ready' | 'notified' | 'cancelled'
...
function parseTurn(value: unknown): AgentSessionTurnView {
  ...
  return {
    ...
    requestId: requireString(record.requestId, 'AGENT_SESSION_VIEW_TURN_REQUEST_ID_INVALID'),
    sourceKind: parseSourceKind(record.sourceKind),
    sourceId: requireString(record.sourceId, 'AGENT_SESSION_VIEW_TURN_SOURCE_ID_INVALID'),
    status: parseTurnStatus(record.status),
    attempt: requireInteger(record.attempt, 'AGENT_SESSION_VIEW_TURN_ATTEMPT_INVALID'),
    ...
    createdAt: requireTimestamp(record.createdAt, 'AGENT_SESSION_VIEW_TURN_CREATED_AT_INVALID'),
    updatedAt: requireTimestamp(record.updatedAt, 'AGENT_SESSION_VIEW_TURN_UPDATED_AT_INVALID'),
  }
}

function parsePendingInteraction(value: unknown): AgentSessionPendingInteractionView | null {
  if (value === null) return null
  const record = requireRecord(value, 'AGENT_SESSION_VIEW_INTERACTION_INVALID')
  const common = {
    interactionId: requireString(record.interactionId, 'AGENT_SESSION_VIEW_INTERACTION_ID_INVALID'),
    turnId: requireString(record.turnId, 'AGENT_SESSION_VIEW_INTERACTION_TURN_ID_INVALID'),
    version: requireInteger(record.version, 'AGENT_SESSION_VIEW_INTERACTION_VERSION_INVALID'),
  ...
  if (record.kind === 'approval') {
    const members = requireArray(record.members, 'AGENT_SESSION_VIEW_APPROVAL_MEMBERS_INVALID').map(
      (value) => {
        const member = requireRecord(value, 'AGENT_SESSION_VIEW_APPROVAL_MEMBER_INVALID')
        return {
          approvalId: requireString(member.approvalId, 'AGENT_SESSION_VIEW_APPROVAL_ID_INVALID'),
          callId: requireString(member.callId, 'AGENT_SESSION_VIEW_APPROVAL_CALL_ID_INVALID'),
  ...
  if (record.kind === 'choice') {
    return {
      ...
      callId: requireString(record.callId, 'AGENT_SESSION_VIEW_CHOICE_CALL_ID_INVALID'),
      card: requireRecord(
        record.card,
...
function parseTask(value: unknown): AgentSessionTaskView {
  ...
  return {
    ...
    taskType: requireString(record.taskType, 'AGENT_SESSION_VIEW_TASK_TYPE_INVALID'),
    targetType: requireString(record.targetType, 'AGENT_SESSION_VIEW_TASK_TARGET_TYPE_INVALID'),
    targetId: requireString(record.targetId, 'AGENT_SESSION_VIEW_TASK_TARGET_ID_INVALID'),
    status: parseTaskStatus(record.status),
    terminal: requireBoolean(record.terminal, 'AGENT_SESSION_VIEW_TASK_TERMINAL_INVALID'),
    errorCode: requireNullableString(
      record.errorCode,
...
function parseBatch(value: unknown): AgentSessionFollowUpBatchView {
  ...
  const progress = requireRecord(record.progress, 'AGENT_SESSION_VIEW_BATCH_PROGRESS_INVALID')
  return {
    batchId: requireString(record.batchId, 'AGENT_SESSION_VIEW_BATCH_ID_INVALID'),
    originTurnId: requireString(
      record.originTurnId,
      'AGENT_SESSION_VIEW_BATCH_ORIGIN_TURN_INVALID',
    ),
    callId: requireString(record.callId, 'AGENT_SESSION_VIEW_BATCH_CALL_ID_INVALID'),
    operationId: requireString(record.operationId, 'AGENT_SESSION_VIEW_BATCH_OPERATION_ID_INVALID'),
    status,
    notifiedTurnId: requireNullableString(
      record.notifiedTurnId,
      'AGENT_SESSION_VIEW_BATCH_NOTIFIED_TURN_INVALID',
    ),
    tasks: requireArray(record.tasks, 'AGENT_SESSION_VIEW_BATCH_TASKS_INVALID').map(parseTask),
    progress: {
      total: requireInteger(progress.total, 'AGENT_SESSION_VIEW_BATCH_TOTAL_INVALID'),
      terminal: requireInteger(progress.terminal, 'AGENT_SESSION_VIEW_BATCH_TERMINAL_INVALID'),
      failed: requireInteger(progress.failed, 'AGENT_SESSION_VIEW_BATCH_FAILED_INVALID'),
      cancelled: requireInteger(progress.cancelled, 'AGENT_SESSION_VIEW_BATCH_CANCELLED_INVALID'),
    },
    createdAt: requireTimestamp(record.createdAt, 'AGENT_SESSION_VIEW_BATCH_CREATED_AT_INVALID'),
    readyAt: requireNullableTimestamp(record.readyAt, 'AGENT_SESSION_VIEW_BATCH_READY_AT_INVALID'),
...
function parseSubagent(value: unknown): AgentSessionSubagentView {
  ...
  requireArray(record.skillReads, 'AGENT_SESSION_VIEW_SUBAGENT_SKILL_READS_INVALID')
  return {
    subagentId: requireString(record.subagentId, 'AGENT_SESSION_VIEW_SUBAGENT_ID_INVALID'),
    taskId: requireString(record.taskId, 'AGENT_SESSION_VIEW_SUBAGENT_TASK_ID_INVALID'),
    originTurnId: requireString(
      record.originTurnId,
      'AGENT_SESSION_VIEW_SUBAGENT_ORIGIN_TURN_INVALID',
    ),
    callId: requireString(record.callId, 'AGENT_SESSION_VIEW_SUBAGENT_CALL_ID_INVALID'),
    outputKind: requireString(
      record.outputKind,
      'AGENT_SESSION_VIEW_SUBAGENT_OUTPUT_KIND_INVALID',
    ) as AgentSessionSubagentView['outputKind'],
    goal: requireString(record.goal, 'AGENT_SESSION_VIEW_SUBAGENT_GOAL_INVALID'),
    status,
    summary: requireNullableString(record.summary, 'AGENT_SESSION_VIEW_SUBAGENT_SUMMARY_INVALID'),
    errorCode: requireNullableString(
      record.errorCode,
...
export async function parseAgentSessionView(value: unknown): Promise<AgentSessionView> {
  ...
  const rawScope = requireRecord(record.scope, 'AGENT_SESSION_VIEW_SCOPE_INVALID')
  const scope: AgentSessionViewScope = {
    projectId: requireString(rawScope.projectId, 'AGENT_SESSION_VIEW_PROJECT_ID_INVALID'),
    userId: requireString(rawScope.userId, 'AGENT_SESSION_VIEW_USER_ID_INVALID'),
    episodeId: requireNullableString(rawScope.episodeId, 'AGENT_SESSION_VIEW_EPISODE_ID_INVALID'),
    assistantId:
  ...
  if (record.thread !== null) {
    const rawThread = requireRecord(record.thread, 'AGENT_SESSION_VIEW_THREAD_INVALID')
    const messages =
    ...
    thread = {
      threadId: requireString(rawThread.threadId, 'AGENT_SESSION_VIEW_THREAD_ID_INVALID'),
  ...
  const currentTurn = record.currentTurn === null ? null : parseTurn(record.currentTurn)
  return {
    protocol: 'agent_session_view_v1',
    scope,
    thread,
    currentTurn,
    queuedTurns: requireArray(record.queuedTurns, 'AGENT_SESSION_VIEW_QUEUED_TURNS_INVALID').map(
      parseTurn,
    ),
    recentTurns: requireArray(record.recentTurns, 'AGENT_SESSION_VIEW_RECENT_TURNS_INVALID').map(
      parseTurn,
    ),
    pendingInteraction: parsePendingInteraction(record.pendingInteraction),
    followUpBatches: requireArray(record.followUpBatches, 'AGENT_SESSION_VIEW_BATCHES_INVALID').map(
      parseBatch,
    ),
    subagents: requireArray(record.subagents, 'AGENT_SESSION_VIEW_SUBAGENTS_INVALID').map(
      parseSubagent,
    ),
  }
}
