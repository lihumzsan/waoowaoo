...
function zhSharedContext(input: {
  readonly story: string
  readonly styleRequest: string
  readonly storyDirection: string
}): string[] {
  return [
    `故事/剧本：${normalize(input.story)}`,
    optionalLine('风格要求：', input.styleRequest),
    optionalLine('故事走向/后续用途：', input.storyDirection),
  ].filter((line): line is string => line !== null)
}

function enSharedContext(input: {
  readonly story: string
  readonly styleRequest: string
  readonly storyDirection: string
}): string[] {
  return [
    `Story/script: ${normalize(input.story)}`,
    optionalLine('Style request: ', input.styleRequest),
    optionalLine('Story direction / later usage: ', input.storyDirection),
  ].filter((line): line is string => line !== null)
}
...
function buildCurrentBaselinePrompt(input: {
  ...
  const prompt = input.locale === 'en'
    ? joinLines([
      ...
      ...enSharedContext(input),
      'Select one concrete location that appears in the story. Prioritize a wide establishing view of the location itself.',
      'Do not explicitly redesign the scene from the style request. Treat style as secondary; focus on visible layout, foreground, midground, background, spatial boundaries, anchor objects, material details, lighting direction, and open standing areas.',
      'No named main characters. No story action. No text labels, numbers, UI marks, watermark, or logo.',
    ])
    : joinLines([
      '使用当前宽广环境参考图逻辑，生成一张空场景资产图。',
      ...zhSharedContext(input),
      '从故事中选择一个具体出现过的地点，优先生成该地点的广角建立视角。',
      '不要主动按风格要求重设计场景，风格只作为次要参考；重点展示可见布局、前景/中景/背景、空间边界、关键锚点、材质细节、光线方向和可落人空地。',
...
function buildStoryCoreSinglePrompt(input: {
  ...
  const prompt = input.locale === 'en'
    ? joinLines([
      ...
      ...enSharedContext(input),
      'Internal decision rule: choose the location that best supports the protagonist conflict, the central turning point, repeated future shots, and the requested style. Do not choose a random mentioned place.',
      'Translate story era, genre, emotional direction, and style request into direct visual traits: architecture, material aging, color palette, lighting source, image filter, atmosphere, and stable negative constraints.',
    ...
    : joinLines([
      ...
      ...zhSharedContext(input),
      '内部选择规则：选择最能承载主角冲突、核心转折、后续反复出镜和当前风格的地点，不能随便挑一个被提到的地点。',
      '把故事时代、类型、情绪走向和风格要求转译成直接可见的视觉属性：建筑结构、材质新旧、色彩方案、灯光来源、画面滤镜、空气氛围和稳定禁用项。',
...
function buildSpatialWidePrompt(input: {
  ...
  const prompt = input.locale === 'en'
    ? joinLines([
      ...
      'Make the scene style concrete: lighting strategy, palette, filter, material texture, weather/air quality, and production-design era must come from the story and style request.',
    ...
    : joinLines([
      ...
      '场景风格必须具体：灯光策略、色彩、滤镜、材质质感、天气/空气感和美术年代感都要来自故事与风格要求。',
...
function buildMultiViewBoardPrompt(input: {
  ...
  const prompt = input.locale === 'en'
    ? joinLines([
      ...
      ...enSharedContext(input),
      'First select the one location that best serves the story arc and style. Then show the same selected location in a single image with three coordinated view regions: main cinematic front view, reverse/back view from the opposite side, and art-directed top-down view.',
    ...
    : joinLines([
      ...
      ...zhSharedContext(input),
      '先选择最能服务故事走向和风格的单一地点，再在同一张图里展示同一场景的三个协调区域：电影化正面主视图、对侧反面视图、美术化顶面视图。',
