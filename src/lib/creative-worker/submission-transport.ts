...
export type CreativeWorkerSubmissionToolSchema = {
  type: 'object'
...
function sanitizeToolSchemaNode(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(sanitizeToolSchemaNode)
  const record = readJsonRecord(value)
  ...
  for (const [key, child] of Object.entries(record)) {
    ...
    if (key === 'const') {
      if (!Object.hasOwn(record, 'enum')) {
        sanitized.enum = [sanitizeToolSchemaNode(child)]
      }
      continue
    }
    if (key === 'oneOf') {
      sanitized.anyOf = sanitizeToolSchemaNode(child)
      continue
    }
    if (key === 'properties') {
      ...
      sanitized.properties = Object.fromEntries(Object.entries(properties).map(([property, schema]) => {
        const propertySchema = sanitizeToolSchemaNode(schema)
    ...
    sanitized[key] = sanitizeToolSchemaNode(child)
  }
  return sanitized
}
...
function schemaMatchesValue(schema: unknown, value: unknown): boolean {
  ...
  if (record.type === 'object') return readJsonRecord(value) !== null
...
function assertStrictObjectShapes(value: unknown, path: string): void {
  ...
  if (record.type === 'object') {
    const properties = readJsonRecord(record.properties)
    if (!properties) throw new Error(`CREATIVE_WORK_SUBMISSION_SCHEMA_PROPERTIES_REQUIRED:${path}`)
    ...
    if (
      ...
      throw new Error(`CREATIVE_WORK_SUBMISSION_SCHEMA_ALL_FIELDS_REQUIRED:${path}`)
    }
    if (record.additionalProperties !== false) {
      throw new Error(`CREATIVE_WORK_SUBMISSION_SCHEMA_ADDITIONAL_PROPERTIES_FORBIDDEN:${path}`)
    }
  }
  for (const [key, child] of Object.entries(record)) {
...
export function buildCreativeWorkerSubmissionToolSchema(input: {
  readonly kind: CreativeWorkOutputKind
  readonly schema: z.ZodObject
}): CreativeWorkerSubmissionToolSchema {
  const generated = z.toJSONSchema(input.schema, { target: 'draft-7' })
  const output = sanitizeToolSchemaNode(generated)
...
export function normalizeCreativeWorkerSubmissionOutput(input: {
  readonly schema: z.ZodObject
