...
const creativeWorkHydratedSourceMaterialSchema = z
  .object({
    label: z
      .string()
      .trim()
      ...
      .max(240)
      .describe(
        'Human-readable name that identifies this source material within the delegated task.',
      ),
    kind: z
      .enum(['text', 'image', 'audio', 'video', 'structured'])
      .describe(
        'The media or data category of the supplied source material; this does not grant the worker access to the underlying project.',
      ),
    content: z
      .string()
      .max(600_000)
      .describe(
        'The complete source content made available to the worker for this run, such as text, a media description, or serialized structured data.',
      ),
    provenance: z
        ...
        z
          .object({
            ...
            resourceId: z
              ...
              .max(32)
              .describe(
                'Globally unique immutable Resource identity; the server resolves all remaining scope and content facts.',
              ),
          })
          .strict(),
        z
      ...
      .describe(
        'Exact provenance for this copied source material, or kind=none when it is not backed by a persisted fact.',
      ),
  })
  .strict()
  .describe(
    'One source item explicitly copied into the stateless worker input by the primary agent.',
  )

const creativeWorkDelegationSourceMaterialSchema = z
  .discriminatedUnion('kind', [
    z
      .object({
        kind: z.literal('resource'),
        resourceId: z
          ...
          .max(32)
          .describe(
            'Globally unique immutable Resource identity. The server reloads its real content, schema, owner, and scope.',
          ),
      })
      .strict(),
    z
  ...
  .describe(
    'Use kind=resource with resourceId only for persisted Resources. Inline material carries its own content and optional exact domain provenance.',
  )

export const creativeWorkDurationIntentSchema = z
    ...
    z
      .object({
        ...
        seconds: z
          ...
          .max(36_000)
          .describe(
            'The exact total delivery duration the user explicitly stated. For video_prompt_set this is the whole work or Chapter duration, never a caller-chosen segment duration or segment count.',
          ),
      })
      .strict()
      .describe(
        'Use only when the user actually stated a delivery duration, or when an adopted plan allocates that stated total to this unit.',
      ),
    z
      .object({
      ...
      .strict()
      .describe(
        'Use whenever the user did not state a delivery duration. The Worker derives the real duration from the dialogue, action, and beats it actually designs.',
      ),
  ])
  .describe(
    'Explicit duration authority for this delegation. There is no implicit default: never convert your own estimate, a planning estimate, or a Chapter estimate into mode=fixed.',
  )

const creativeWorkRequestBaseShape = {
  outputKind: z
    .enum(CREATIVE_WORK_OUTPUT_KINDS)
  ...
  durationIntent: creativeWorkDurationIntentSchema
    .optional()
    .describe(
      'Required for video_prompt_set. Omit only for output kinds whose result has no delivery duration.',
    ),
} as const

export const creativeWorkDelegationRequestSchema = z
  .object({
    ...
    context: z
      .object({
        userRequest: z
          ...
          .max(30_000)
          .describe(
            'The relevant original user request, preserved so the worker can remain faithful to user intent.',
          ),
        sourceMaterials: z
          .array(creativeWorkDelegationSourceMaterialSchema)
          .max(64)
          .describe(
            'Exact Resource IDs or inline non-Resource materials. Persisted Resource content is always reloaded by the server.',
          ),
        constraints: z
      ...
      .strict()
      .describe(
        'A complete caller-assembled context packet; it is data for analysis and grants no system access.',
      ),
  })
  .strict()
  .describe('Request for one isolated creative-worker run with a strict output contract.')

export const creativeWorkHydratedRequestSchema = z
  .object({
    ...creativeWorkRequestBaseShape,
    context: z
      .object({
...
const creativeMusicProductionContextSchema = z
  .object({
...
const creativeVideoProductionContextSchema = z
  .object({
...
export const creativeWorkRequestSchema = creativeWorkHydratedRequestSchema
  .extend({
    creativeDirection: injectedCreativeDirectionSchema
      .nullable()
      .describe(
        'Complete exact adopted Creative Direction injected and frozen by the server for every output kind except creative_direction itself. The primary Agent cannot supply it.',
      ),
    productionContext: z
      .object({
  ...
  .strict()
  .describe(
    'Server-compiled request for one isolated creative-worker run. Creative Direction and productionContext are supplied by the execution layer, never by the primary Agent.',
  )
...
export type CreativeWorkerEventListener = (event: CreativeWorkerEvent) => void | Promise<void>
...
export interface RunCreativeWorkerInput {
  ...
  /**
   * Reports the cumulative SDK usage as soon as the model run settles, before
   * any local output, Skill, or lifecycle validation can fail. The Task
   * handler owns durable persistence; the stateless Worker only exposes the
   * provider fact it observed.
   */
  onUsageObserved?: (usage: LlmUsageFact) => void
  webSearch?: (input: {
...
export type CreativeWorkerExecutionResult<TOutput = CreativeWorkOutput> =
  CreativeWorkerResult<TOutput> & {
    usage: LlmUsageFact
  }
