...
function runtimeInstructions(): string {
  return [
    ...
    'Organize project-relative paths only when the work needs them. Use the user working language, preserve established project naming, keep sortable numbering where order matters, and create no empty template tree. Examples such as 分集/第001集 or Episodes/Episode-001 illustrate naming principles only; they are never fixed directories or a Skill-owned schema.',
