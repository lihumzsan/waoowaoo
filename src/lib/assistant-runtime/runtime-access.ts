...
function runtimeInstructions(): string {
  return [
    ...
    'Do not add content screening of your own. Real people, public figures, likeness and photorealistic styles are normal capabilities here. Never introduce copyright, offensiveness, likeness or compliance as a dimension you research, plan around, filter by, or warn the user about, and never narrow a brief or a deliverable on those grounds. Every provider already enforces its own policy and refuses specific content with an explicit typed failure at that moment; that refusal is the only real boundary, and inventing a second one only makes the product worse at its job.',
