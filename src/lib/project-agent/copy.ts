...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'Only after the screenplay exists can screenplay-based style preview images exist. After those style previews are ready and before calling generate_edit_director_decoupage, call request_edit_first_choice with choiceType="style" and wait for the user to click the card. Do not ask them to type the style, and do not invent or render card JSON in text.',
      ...
      'When the user asks for AI edit, short-film generation, or an edit-first video, call get_project_context/get_project_phase first. If no ready editScreenplay exists, first ensure duration and aspect ratio have been selected with request_edit_first_choice when needed, then use generate_edit_screenplay. If no ready director decoupage exists, first ensure screenplay-based style preview images are ready and visual style is selected with request_edit_first_choice when needed, then use generate_edit_director_decoupage. If no ready editScript exists, use generate_edit_script. After required assets and spatial profiles are ready, use generate_edit_cinematography_shot_plan before storyboard generation.',
  ...
  return [
    ...
    '剪辑先行制作的产物依赖顺序是：时长+画面比例选择 -> 剧本 -> 基于剧本生成风格候选图 -> 视觉风格选择 -> 导演拆镜 -> 剪辑先行表 -> 需求资产/空间档案 -> 摄影 shot plan -> 分镜面板/图片 -> 视频片段 -> 最终成片。这是产物依赖规则，不是前端写死流程。',
    ...
    '只有剧本生成后，才有基于剧本的风格候选图。风格候选 ready 后、调用 generate_edit_director_decoupage 前，先调用 request_edit_first_choice 并传 choiceType="style"，然后等待用户点击卡片；不要要求用户用文字输入风格，也不要在文本里编造或渲染卡片 JSON。',
    ...
    '当用户要求 AI 剪辑、生成短片或剪辑先行视频时，必须先调用 get_project_context/get_project_phase。若没有 ready 的 editScreenplay，先按需用 request_edit_first_choice 确认时长和画面比例，再调用 generate_edit_screenplay；若没有 ready 的导演拆镜，先确认基于剧本的风格候选图已经 ready，并按需用 request_edit_first_choice 确认视觉风格，再调用 generate_edit_director_decoupage；若没有 ready 的 editScript，调用 generate_edit_script；需求资产和空间档案 ready 后，必须先调用 generate_edit_cinematography_shot_plan，再生成分镜。',
