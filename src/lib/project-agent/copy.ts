...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'When the user asks for AI edit, short-film generation, or an edit-first video, call get_project_context/get_project_phase first. If no ready editScreenplay exists, use generate_edit_screenplay. If no ready director decoupage exists, use generate_edit_director_decoupage. If no ready editScript exists, use generate_edit_script. After required assets and spatial profiles are ready, use generate_edit_cinematography_shot_plan before storyboard generation.',
  ...
  return [
    ...
    '当用户要求 AI 剪辑、生成短片或剪辑先行视频时，必须先调用 get_project_context/get_project_phase。若没有 ready 的 editScreenplay，先调用 generate_edit_screenplay；若没有 ready 的导演拆镜，调用 generate_edit_director_decoupage；若没有 ready 的 editScript，调用 generate_edit_script；需求资产和空间档案 ready 后，必须先调用 generate_edit_cinematography_shot_plan，再生成分镜。',
