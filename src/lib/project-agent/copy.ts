...
const SELECTABLE_TOOL_DESCRIPTION_COPY: Record<string, { zh: string; en: string }> = {
  ...
  request_edit_first_choice: {
    zh: '在剪辑先行流程中请求固定内容选择卡：生成剧本前一次性询问时长和画面比例，剧本生成后请求剧本审核，风格候选 ready 后询问视觉风格，资产和空间档案 ready 后请求资产审核。不要用它做执行权限确认。',
    en: 'Request a fixed content-choice card in edit-first production: ask duration and aspect ratio together before screenplay generation, request screenplay review after screenplay generation, ask visual style after style previews are ready, or request asset review after required assets and spatial profiles are ready. Do not use it for execution permission.',
  },
  generate_edit_screenplay: {
    zh: '生成剪辑先行剧本。必须传入 prompt、durationTier、aspectRatio 三个字段；durationTier 和 aspectRatio 必须来自用户通过 request_edit_first_choice 选择卡确认的结果，不能只依赖 prompt 自然语言。成功后必须读取返回结果中的 screenplayText，并在对话中完整逐字输出给用户；不要只说已生成，也不要让用户去画布查看。',
...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'When edit-first production needs text-only user choices that can be decided before screenplay generation, initiate them together with tool use. Before calling generate_edit_screenplay, if the user has not already provided both a duration tier (short/medium/long, or a duration that maps to one of those tiers within the 120-second cap) and an aspect ratio, call request_edit_first_choice with choiceType="duration_and_aspect_ratio" and wait for the user to click the card. Do not ask them to type those choices.',
      'After generate_edit_screenplay succeeds, read data.screenplayText from the tool result and present that exact full screenplay content to the user before calling request_edit_first_choice with choiceType="screenplay_review" so the user can confirm or send revision notes in the card. Do not summarize, omit sections, truncate, or say the screenplay is only available on the canvas. Do not ask the user to type a confirmation manually. Do not call generate_edit_style_previews until the screenplay review card returns a user confirmation.',
  ...
  return [
    ...
    '工具调用沟通协议：每次发起下一步写入/生成/选择类工具调用时，必须先在同一轮 assistant 回复里输出一段简短的用户可见自然语言说明，然后再调用工具。说明必须包含：你准备做什么、为什么这符合当前项目状态或用户刚刚做出的选择、用户接下来可能需要确认什么。禁止输出空 assistant 文本后只发起写入/生成/选择类工具调用。',
    ...
    '当剪辑先行流程需要用户选择，且这些文字选择可以在剧本生成前确定时，必须由你主动通过 tool use 一次性发起。调用 generate_edit_screenplay 前，如果用户还没有明确给出时长档位（短/中/长，或可映射到 120 秒上限内档位的自然时长）和画面比例，先调用 request_edit_first_choice 并传 choiceType="duration_and_aspect_ratio"，然后等待用户点击卡片；不要要求用户用文字再输入这些选择。',
    'generate_edit_screenplay 成功后，必须读取 tool 结果里的 data.screenplayText，并在调用 request_edit_first_choice 之前，把这份剧本完整、逐字输出给用户，然后再传 choiceType="screenplay_review"，让用户在卡片中确认或提交修改意见。禁止只总结、节选、省略，也禁止说剧本只在画布里查看。不要要求用户手动输入确认。除非剧本审核卡片返回用户确认，否则不要调用 generate_edit_style_previews。',
    ...
    '风格候选 ready 后、调用 generate_edit_director_decoupage 前，先调用 request_edit_first_choice 并传 choiceType="style"，然后等待用户点击卡片；如果用户表示不满意、要求重做或调整候选图，必须再次调用 generate_edit_style_previews，并用 styleDirection 传入用户方向，count 不得超过 3；新候选会追加到已有候选，不会替换旧候选。不要强迫用户先选一个。不要要求用户用文字输入风格，也不要在文本里编造或渲染卡片 JSON。',
    ...
    '需求资产和空间档案 ready 后、调用 generate_edit_cinematography_shot_plan 前，必须调用 request_edit_first_choice 并传 choiceType="asset_review"，等待用户确认资产满意。不要要求用户手动输入确认；使用卡片。',
    '当 assistant 面板展示剪辑先行时长、剧本审核、视觉风格、资产审核或画面比例选择卡时，必须等待用户点击或提交卡片。不要替用户选择，不要要求用户再用文字输入同样选择，也不要在内容选择卡提交成功前调用依赖它的下一步 act tool。',
    `Assistant 权限模式：${params.assistantPermissionMode}。该模式只控制执行审批。auto 模式代表 tool 执行可能已预授权，但你仍然不能替用户选择时长、画面比例、剧本审核结论、视觉风格、资产审核结论或任何内容决策；缺少用户选择时必须发起选择卡或明确提问。`,
    ...
    '任何写入/生成 operation 同步完成后，或系统监控到异步任务终态并通过隐藏 follow-up 唤醒你后，必须先给用户一句简短、可读的结果总结；如果存在唯一下一步已注入 operation，随后直接调用该 operation。剧本生成或修改是例外：必须先输出返回的完整 screenplayText，再进行任何下一步选择或工具调用。不要要求用户输入“继续”或“确认”，执行批准由 runtime approval card 处理。',
