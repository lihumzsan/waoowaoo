...
const SELECTABLE_TOOL_DESCRIPTION_COPY: Record<string, { zh: string; en: string }> = {
  ...
  generate_edit_screenplay: {
    zh: '生成剪辑先行剧本。必须传入 prompt、durationTier、aspectRatio 三个字段；durationTier 和 aspectRatio 必须来自用户通过 request_edit_first_choice 选择卡确认的结果，不能只依赖 prompt 自然语言。',
    en: 'Generate the edit-first screenplay. You must pass prompt, durationTier, and aspectRatio. durationTier and aspectRatio must come from the user selection confirmed through request_edit_first_choice; do not rely on prompt text alone.',
  },
  revise_edit_screenplay: {
    zh: '修改当前剪辑先行剧本。仅在剧本已生成、用户尚未确认进入风格候选/导演拆镜/剪辑表前使用。用户要求调整剧情、题材、氛围、结构、角色、结尾或表达方向时调用；必须传入 revisionInstruction、durationTier、aspectRatio，修改后仍停留在剧本审核阶段。',
