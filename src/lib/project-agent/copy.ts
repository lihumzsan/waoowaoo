...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'When confirmationRequired=true: wait for the runtime approval card result. Do not ask the user to type confirmation and do not call request_edit_first_choice for execution permission. After approval, denial, failure, or completion, the result will return through the original tool result chain.',
  ...
  return [
    ...
    '当 tool 返回 confirmationRequired=true：等待 runtime 批准卡结果。不要要求用户手动输入确认，也不要用 request_edit_first_choice 做执行权限确认。批准、拒绝、失败或完成后，结果会通过原 tool result 链路返回。',
