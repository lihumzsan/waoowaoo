...
const SELECTABLE_TOOL_DESCRIPTION_COPY: Record<string, { zh: string; en: string }> = {
  ...
  request_edit_first_choice: {
    zh: '在剪辑先行流程中请求固定选择卡：生成剧本前一次性询问时长和画面比例，剧本生成后请求剧本审核，风格候选 ready 后询问视觉风格，后续每个唯一下一步 operation 前请求下一步确认。',
    en: 'Request a fixed choice card in edit-first production: ask duration and aspect ratio together before screenplay generation, request screenplay review after screenplay generation, ask visual style after style previews are ready, or request next-step confirmation before each later immediate-next operation.',
  },
...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'After the visual style is selected, every later immediate-next edit-first operation must be confirmed with a card before execution. If the current workflow nextAction is director decoupage, edit script/core edit table, required assets, cinematography shot plan, storyboard, videos, or final render, call request_edit_first_choice with choiceType="next_step_confirmation" and wait for the user to click the card. Do not ask the user to type "continue" manually.',
      'When the assistant panel displays a choice card for edit-first duration, screenplay review, visual style, aspect ratio, or next-step confirmation, wait for the user to click or submit that card. Do not choose for the user, do not ask them to type the same choice, and do not call the next act tool until the card submission returns.',
      'For edit-first production, treat that dependency order as a dependency gate. Before any act tool call, read project state and identify the earliest missing required artifact. You may only prepare, request confirmation for, execute the operation that creates the immediate next artifact, or repair/regenerate the current-stage artifact the user is responding to.',
  ...
  return [
    ...
    '视觉风格选中后，后续每一个当前唯一下一步 edit-first operation 都必须先用卡片确认。若当前 workflow nextAction 是导演拆镜、剪辑先行表/剪辑核心表、需求资产、摄影 shot plan、分镜、视频片段或最终成片，先调用 request_edit_first_choice 并传 choiceType="next_step_confirmation"，等待用户点击卡片；不要要求用户手动输入“继续”。',
    '当 assistant 面板展示剪辑先行时长、剧本审核、视觉风格、画面比例或下一步确认卡时，必须等待用户点击或提交卡片。不要替用户选择，不要要求用户再用文字输入同样选择，也不要在卡片提交成功前调用下一步 act tool。',
    '剪辑先行制作必须把上述产物依赖顺序当作产物依赖约束。每次调用 act tool 前，必须先读取项目状态并识别最早缺失的必要产物；你只能准备、请求确认、执行创建当前下一步产物，或修复/重生成用户正在反馈的当前阶段产物。',
