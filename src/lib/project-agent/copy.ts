...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'Before initiating a next-step tool call, write a brief user-facing explanation in the same assistant turn describing what you are about to do and why it is the correct next step, then call the tool. Do not mention internal operation names, tool ids, workflow gates, or approval implementation details in that explanation.',
  ...
  return [
    ...
    '发起下一步工具调用前，必须在同一轮 assistant 回复里先用简短的用户可见自然语言说明你准备做什么，以及为什么这是正确的下一步，然后再调用工具。这段说明禁止提内部 operation 名、tool id、workflow 门禁或审批实现细节。',
