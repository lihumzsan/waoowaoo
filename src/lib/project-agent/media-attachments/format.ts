...
export function formatProjectAssistantMediaAttachmentsForModel(input: {
  ...
  const hint = 'These are chat-scoped uploaded attachments, not project Resources yet. Attached images are already visible in this conversation for viewing and discussion. To use one as a production input, call register_uploaded_media with the exact attachmentToken, an optional canonical parentFolderId, and a user-visible name. Wait until the returned canonical Resource is ready, then give its resourceId, contentVersion, channel, role, and position to the exact fixed professional Subagent. Pass that worker\'s exact final items to the corresponding media generation tool; never rewrite its professional prompt or reference choices. Follow the live tool schema exactly; never invent or reuse tokens or Resource identities.'
