...
function listLocalCodexExecutableCandidates(): string[] {
  ...
  try {
    ...
    return [path.join(binDir, 'codex.exe')]
  }
...
export function resolveCodexExecutablePath(rawPath?: string): string {
  ...
  const defaultAutoPath = expandWindowsEnv(CODEX_DEFAULT_EXECUTABLE_PATH)
  const shouldAutoResolve = !rawPath?.trim() || samePath(withEnv, defaultAutoPath)
  ...
  return new Promise((resolve, reject) => {
    ...
    const requestTermination = () => {
      try {
        ...
        // Timeout rejection below is the caller-visible failure.
      }
      ...
      killTimer = setTimeout(() => {
        try {
          ...
          // forceRejectTimer guarantees the caller is released.
        }
...
function extractJsonObjects(text: string): unknown[] {
  ...
  try {
    ...
    // Scan line-delimited and embedded JSON objects below.
  }
