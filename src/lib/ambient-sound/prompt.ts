...
function buildTimelinePayload(clips: readonly FinalRenderClipPlan[]): unknown {
  return clips.map((clip) => ({
    ...
    nativeVideoSynchronizedSound: clip.synchronousSound,
  }))
}
...
export function buildAmbientSoundPlanPrompt(input: {
  ...
  if (input.locale === 'zh') {
    return [
      '你是专业影视声音设计师，只为 AI 生成视频规划连续环境音（ambient sound bed）。',
      ...
      '7. source prompt 必须要求 seamless loop，并明确 no music, no voices, no dialogue, no footsteps, no impacts；loopDurationSeconds 必须固定输出 12。',
  ...
  return [
    'You are a professional film sound designer planning only the continuous ambient sound bed for an AI-generated video.',
    ...
    '7. Every source prompt must request a seamless loop and explicitly say no music, no voices, no dialogue, no footsteps, no impacts; loopDurationSeconds must always be 12.',
