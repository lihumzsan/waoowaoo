...
import type {
  AmbientSoundIntensity,
  AmbientSoundPerspective,
  AmbientSoundSourceAsset,
  AmbientSoundTransition,
} from './types'
import type { AmbientSoundResolvedSection } from './timeline'

export const AMBIENT_SOUND_AUDIO_TARGET_LUFS = -24
export const AMBIENT_SOUND_TRANSITION_SECONDS = 0.75

const PERSPECTIVE_DSP: Record<AmbientSoundPerspective, {
...
const INTENSITY_GAIN_DB: Record<AmbientSoundIntensity, number> = {
  low: -8,
  medium: 0,
  high: 4,
}
...
function formatFilterNumber(value: number): string {
  if (!Number.isFinite(value)) throw new Error('AMBIENT_SOUND_FILTER_NUMBER_INVALID')
...
function transitionDuration(transition: AmbientSoundTransition): number {
  return transition === 'cut' ? 0 : AMBIENT_SOUND_TRANSITION_SECONDS
}

function buildFadeFilters(section: AmbientSoundResolvedSection): string[] {
  ...
  if (fadeInSeconds + fadeOutSeconds >= section.durationSeconds) {
    throw new Error(`AMBIENT_SOUND_SECTION_TRANSITION_TOO_LONG:${section.section.fromShotId}:${section.section.toShotId}`)
  }
...
export function buildAmbientSoundSectionDspFilters(section: AmbientSoundResolvedSection): string[] {
...
function sourceKey(source: Pick<AmbientSoundSourceAsset, 'sourceId'>): string {
  return source.sourceId
}

function buildSourceMaps(input: {
  readonly sources: readonly AmbientSoundSourceAsset[]
  readonly sections: readonly AmbientSoundResolvedSection[]
}): {
  readonly sourceById: ReadonlyMap<string, AmbientSoundSourceAsset>
  ...
  for (const section of input.sections) {
    ...
    if (!sourceById.has(id)) throw new Error(`AMBIENT_SOUND_SECTION_SOURCE_MISSING:${id}`)
...
export function buildAmbientSoundFilterComplex(input: {
  readonly sources: readonly AmbientSoundSourceAsset[]
  readonly sections: readonly AmbientSoundResolvedSection[]
  readonly durationSeconds: number
}): string {
  if (input.sources.length === 0) throw new Error('AMBIENT_SOUND_MIX_SOURCES_EMPTY')
  if (input.sections.length === 0) throw new Error('AMBIENT_SOUND_MIX_SECTIONS_EMPTY')
  if (!Number.isFinite(input.durationSeconds) || input.durationSeconds <= 0) {
    throw new Error('AMBIENT_SOUND_MIX_DURATION_INVALID')
  }
  ...
  for (const section of input.sections) {
    ...
    if (sourceIndex === undefined || sourceOrdinal === undefined) {
      throw new Error(`AMBIENT_SOUND_SECTION_SOURCE_RESOLUTION_FAILED:${section.section.sourceId}`)
    }
    ...
    const sectionFilters = [
      ...
      ...buildAmbientSoundSectionDspFilters(section),
  ...
  const mixFilters = [
    ...
    'asetpts=PTS-STARTPTS',
    `loudnorm=I=${AMBIENT_SOUND_AUDIO_TARGET_LUFS}:TP=-2:LRA=11`,
    'alimiter=limit=0.95',
  ].join(',')
...
export async function renderAmbientSoundMix(input: {
  ...
  if (input.sourcePaths.length !== input.sources.length) {
    throw new Error('AMBIENT_SOUND_MIX_SOURCE_PATH_COUNT_INVALID')
  }
  ...
  await input.runCommand('ffmpeg', [
    ...
    ...sourceInputs,
    '-filter_complex',
    buildAmbientSoundFilterComplex({
