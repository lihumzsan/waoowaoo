...
import type { AmbientSoundPlan, AmbientSoundPlanSection } from './types'

export interface AmbientSoundResolvedSection {
  readonly section: AmbientSoundPlanSection
...
function ensureTimeline(clips: readonly FinalRenderClipPlan[]): void {
  if (clips.length === 0) throw new Error('AMBIENT_SOUND_TIMELINE_EMPTY')
  const invalid = clips.find((clip) => !Number.isFinite(clip.durationSeconds) || clip.durationSeconds <= 0)
  if (invalid) throw new Error(`AMBIENT_SOUND_TIMELINE_CLIP_DURATION_INVALID:${invalid.sourceId}`)
}

export function buildAmbientSoundTimelineSignature(clips: readonly FinalRenderClipPlan[]): string {
...
function findClipIndexForShotId(clips: readonly FinalRenderClipPlan[], shotId: string): number {
  ...
  if (index === -1) throw new Error(`AMBIENT_SOUND_SHOT_ANCHOR_NOT_FOUND:${shotId}`)
  return index
}
...
export function resolveAmbientSoundSectionTimeline(input: {
  readonly clips: readonly FinalRenderClipPlan[]
  readonly plan: AmbientSoundPlan
}): AmbientSoundResolvedSection[] {
  ...
  return input.plan.sections.map((section, index) => {
    ...
    if (toIndex < fromIndex) {
      throw new Error(`AMBIENT_SOUND_SHOT_ANCHOR_ORDER_INVALID:${section.fromShotId}:${section.toShotId}`)
    }

    const startSeconds = clipStarts[fromIndex]
    const toClip = input.clips[toIndex]
    if (startSeconds === undefined || !toClip) {
      throw new Error('AMBIENT_SOUND_SHOT_ANCHOR_RESOLUTION_FAILED')
    }
    const endSeconds = clipStarts[toIndex] + toClip.durationSeconds
    const durationSeconds = endSeconds - startSeconds
    if (!Number.isFinite(durationSeconds) || durationSeconds <= 0) {
      throw new Error(`AMBIENT_SOUND_SECTION_DURATION_INVALID:${section.fromShotId}:${section.toShotId}`)
    }
