...
export default function EstimatedTaskProgressOverlay({
  ...
  if (progress.isFailed) {
    return (
      <div
        className={[
          'absolute inset-0 z-20 flex items-center justify-center bg-[rgba(38,16,16,0.62)] px-4 text-white backdrop-blur-sm',
          className || '',
        ].join(' ').trim()}
      >
        <div className="flex items-center gap-2 rounded-lg border border-white/16 bg-[rgba(127,29,29,0.62)] px-3 py-2 shadow-[0_12px_28px_rgba(0,0,0,0.2)]">
  ...
  return (
    <div
      className={[
        'absolute inset-0 z-20 flex items-center justify-center bg-[rgba(10,16,30,0.44)] px-4 text-white backdrop-blur-sm',
        className || '',
      ].join(' ').trim()}
    >
      <div className="w-full max-w-[178px] rounded-lg border border-white/18 bg-[rgba(12,18,32,0.5)] p-3 shadow-[0_14px_34px_rgba(0,0,0,0.22)]">
