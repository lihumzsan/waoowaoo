...
/* eslint-disable no-restricted-syntax -- Preserve Beautiful UI's copied inline glyphs exactly. */
...
export type ChatComposerCopy = {
  tabs: [string, string];
  initialPrompt: string;
  first: { label: string; sub: string; time: string; body: string };
  second: { label: string; sub: string; time: string; body: string };
...
const DEFAULT_COPY: ChatComposerCopy = {
  tabs: ["Flavors", "Suppliers"],
  initialPrompt: "Compare mint chip to last summer",
  first: { label: "Sales History", sub: "Flavor Data", time: "4s", body: "Pulled 3 summers of mint chip sales for comparison." },
  second: { label: "Comparison", sub: "Trend Detection", time: "2s", body: "Mint chip is up 12% with stronger weekend peaks." },
  durationJoiner: "for",
  action: "Action",
  placeholder: "Prompt or tag a flavor with @",
  inputLabel: "Chat prompt",
  send: "Send",
};
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 text-[12px] leading-[1.3]">
        ...
        <span className="text-ink">{durationJoiner} {time}</span>
      </div>
...
export default function ChatComposer({
  ...
  const [submitted, setSubmitted] = useState(copy.initialPrompt);
  const [tab, setTab] = useState(copy.tabs[0]);
  ...
  return (
    <div className={`flex w-full max-w-95 flex-col self-start overflow-hidden rounded-[14px] bg-surface shadow-card ${composerOnly ? "h-auto" : "h-[288px]"}`}>
      {/* header — tabs + actions */}
      {!composerOnly ? <div className="flex shrink-0 items-center justify-between border-b border-line p-1.5">
      ...
      {!composerOnly ? <div className="flex min-h-0 flex-1 flex-col gap-2.5 overflow-y-auto px-3 pt-2.5 pb-1">
        ...
        {phase === "reply1" || phase === "reply2" || phase === "done" ? (
          <Section
            ...
            sub={copy.first.sub}
            time={copy.first.time}
            durationJoiner={copy.durationJoiner}
            body={copy.first.body}
          />
        ) : null}
        {phase === "reply2" || phase === "done" ? (
          <Section
            ...
            sub={copy.second.sub}
            time={copy.second.time}
            durationJoiner={copy.durationJoiner}
            body={copy.second.body}
