...
/* eslint-disable no-restricted-syntax, @next/next/no-img-element -- Preserve the copied Beautiful UI glyphs and data-URI source avatars. */
...
export default function StreamingText({
  ...
  const words = text?.split(" ").map((value) => ({ text: value })) ?? TOKENS;
  const tokens: Token[] = text
    ? [...words.slice(0, 12), { text: "", cite: true }, ...words.slice(12)]
    : TOKENS;
    ...
    <div className="min-h-[15.5rem] w-full max-w-95">
      <p className="text-[13px] leading-relaxed text-ink">
        {tokens.slice(0, count).map((token, i) =>
          token.cite ? (
            <SourceChip key={i} source={sources[0] ?? SOURCES[0]} />
          ) : (
            <span
              key={i}
      ...
      >
        ...
        >
          ...
          <span className="text-[12px] text-ink-2">{sourceCountLabel}</span>
        </button>
      </div>
