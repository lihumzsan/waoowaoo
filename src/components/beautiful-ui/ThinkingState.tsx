...
export type ThinkingStateData = {
  active: string;
  done: string;
  rows: ThinkingStateRow[];
  query?: string;
  moreLabel?: string;
};
...
const VARIANTS: Record<
  string,
  { active: string; done: string; rows: ThinkingStateRow[]; query?: string }
> = {
  Steps: {
    active: "Thinking",
...
export default function ThinkingState({
  ...
  variant?: string;
  data?: ThinkingStateData;
}) {
  const stage = useSequence(STAGES);
  const [manualExpanded, setManualExpanded] = useState<boolean | null>(null);
  const [selectedTool, setSelectedTool] = useState<string | null>(null);
  const v: ThinkingStateData = data ?? VARIANTS[variant] ?? VARIANTS.Steps;
