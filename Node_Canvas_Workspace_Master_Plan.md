...
src/
  ...
  features/project-workspace/
    ...
    canvas/
      ...
      details/
        CanvasObjectDetailLayer.tsx       # 画布内对象详情层 shell，只负责选中对象分发和命令桥
        StoryDetail.tsx                   # 故事输入、AI 帮写、长文本检测、智能分集、比例/风格配置
        ScriptClipDetail.tsx              # 剧本片段、原始片段、clip 资产引用编辑
        ShotDetail.tsx                    # 分镜字段、自动保存、插入、变体、AI Data、错误清理
        ImageDetail.tsx                   # 图片预览、候选、参考图、编辑、撤回、下载
        VideoDetail.tsx                   # 视频预览、capability 控件、首尾帧、批量/下载
        FinalDetail.tsx                   # 非 AI 剪辑成片统计与批量入口
        detail-shared.tsx                 # 详情层共享 UI / 工具函数
  lib/project-canvas/
...
### 阶段 5: 节点画布功能等价迁移（不含语音 / Lip Sync / AI 剪辑）

- ✅ **Task 5.1**: `src/features/project-workspace/canvas/details/CanvasObjectDetailLayer.tsx`、`details/StoryDetail.tsx`、`details/ScriptClipDetail.tsx`、`details/ShotDetail.tsx`、`details/ImageDetail.tsx`、`details/VideoDetail.tsx`、`details/FinalDetail.tsx`、`details/detail-shared.tsx` - 将详情层从单文件拆为按对象类型的模块；shell 只负责 selected node 分发、统一命令桥和刷新，不恢复旧 stage tab，不把旧页面级 UI 原样塞进节点。
...
- ✅ **Task 5.9**: `details/ImageDetail.tsx` - 图片详情恢复图片预览/大图预览、候选图确认/取消、参考图选择（分镜、角色、场景、道具、自定义 URL、上传/粘贴、notes）、数量选择、重新生成、图片编辑弹层、批量下载、清错误；生成中显示强制重新生成文案但仍走同一个真实生成 operation。
- ✅ **Task 5.10**: `src/lib/operations/domains/storyboard/panel-mutations.ts`、`src/lib/operations/domains/storyboard/panel-edit-ops.ts`、`src/app/api/projects/[projectId]/panel/revert-image/route.ts`、`src/lib/query/mutations/storyboard-panel-mutations.ts` - 新增真实 panel image revert operation / route / hook；只有存在 `previousImageUrl` 或 `previousImageMedia` 时 UI 显示撤回按钮，失败显式报错，不再显示“无接口只读提示”。
- ✅ **Task 5.11**: `details/VideoDetail.tsx` - 视频详情恢复播放器、图片 fallback、video prompt 保存、视频模型选择、capability 控件（不再手写 JSON）、缺失 capability 禁用和提示、单镜头生成、首尾帧 link/unlink、首尾帧 prompt、FL 模型/capability、批量生成、zip 下载；明确移除 lip-sync / voice UI。
- ✅ **Task 5.12**: `details/FinalDetail.tsx` - 成片详情保留非 AI 剪辑能力：视频顺序、总镜头、总视频、缺失视频、总时长、批量生成视频、下载全部视频 zip；不接入 `features/video-editor/**`，不显示进入剪辑器/导出最终成片按钮。
- ✅ **Task 5.13**: `src/features/project-workspace/canvas/hooks/useWorkspaceNodeCanvasProjection.ts`、`src/features/project-workspace/canvas/nodes/WorkspaceNode.tsx`、`src/features/project-workspace/canvas/node-canvas-types.ts` - 从新节点画布投影和节点表面移除 lip-sync 字段、lip-sync 错误和 lip-sync 成为视频节点的判断依据；本轮不生成 voice/lip-sync 节点或入口。
- ✅ **Task 5.14**: `messages/en/project-workflow.json`、`messages/zh/project-workflow.json` - 新增详情层全部正式文案；禁止硬编码中文。
...
BILLING_TEST_BOOTSTRAP=0 npm exec -- vitest run tests/integration/api/contract/project-panel-routes.test.ts tests/integration/api/contract/project-download-routes.test.ts
```
