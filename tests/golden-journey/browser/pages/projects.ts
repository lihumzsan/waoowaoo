...
export async function openGoldenProjectList(page: Page): Promise<void> {
  await page.goto('/zh/workspace')
  await expect(page.getByText('新建项目', { exact: true }).first()).toBeVisible({ timeout: 30_000 })
}

export async function createGoldenProjectThroughUi(page: Page, input: {
  ...
  if (response.status() !== 201) {
    throw new Error(`GOLDEN_PROJECT_CREATE_HTTP_${String(response.status())}`)
  }
  if (preferenceResponse.status() !== 200) {
    throw new Error(`GOLDEN_PROJECT_CREATE_PREFERENCE_HTTP_${String(preferenceResponse.status())}`)
  }
...
export async function editGoldenProjectThroughUi(page: Page, input: {
  ...
  if (response.status() !== 200) {
    throw new Error(`GOLDEN_PROJECT_EDIT_HTTP_${String(response.status())}`)
  }
  await expect(projectLink(page, input.projectId)).toContainText(input.name)
}

export async function searchGoldenProjects(page: Page, query: string): Promise<void> {
  ...
  if ((await response).status() !== 200) throw new Error('GOLDEN_PROJECT_SEARCH_FAILED')
}

export async function deleteGoldenProjectThroughUi(page: Page, input: {
  readonly projectId: string
  readonly name: string
}): Promise<void> {
  await openGoldenProjectList(page)
  ...
  if (response.status() !== 200) {
    throw new Error(`GOLDEN_PROJECT_DELETE_HTTP_${String(response.status())}`)
  }
  if (refreshResponse.status() !== 200) {
    throw new Error(`GOLDEN_PROJECT_DELETE_REFRESH_HTTP_${String(refreshResponse.status())}`)
  }
