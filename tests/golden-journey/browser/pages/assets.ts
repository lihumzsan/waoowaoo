...
export async function importGoldenGlobalCharacterThroughUi(page: Page, input: {
  ...
  if (response.status() !== 200) {
    throw new Error(`GOLDEN_GLOBAL_CHARACTER_IMPORT_HTTP_${String(response.status())}`)
  }
  await expect(picker).toHaveCount(0)
}

export async function editGoldenGlobalCharacterDescriptionThroughUi(page: Page, input: {
  ...
  if (updateResponse.status() !== 200) {
    throw new Error('GOLDEN_GLOBAL_CHARACTER_DESCRIPTION_UPDATE_FAILED')
  }
  await expect(modal).toHaveCount(0)
}

export async function deleteGoldenGlobalCharacterThroughUi(page: Page, input: {
  readonly characterId: string
}): Promise<void> {
  await page.goto('/zh/workspace/asset-hub', { waitUntil: 'domcontentloaded' })
  const card = page.getByTestId(`global-character-${input.characterId}`)
  ...
  if ((await response).status() !== 200) {
    throw new Error('GOLDEN_GLOBAL_CHARACTER_DELETE_FAILED')
  }
