...
test('[GJ-MODEL-STOPS-AFTER-CONFIRM] orchestrator continues after model stops', async ({ page }, testInfo) => {
  ...
  if (scriptOutcome !== 'script_review') {
    await attachGoldenOracleEvidence(testInfo, scope, 'golden-oracle-model-stop-before-script-review')
  }
  expect(scriptOutcome).toBe('script_review')
