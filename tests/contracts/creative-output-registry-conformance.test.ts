...
describe('Creative output registry conformance', () => {
  it('binds every creative domain to exactly one Skill, outputKind, strict schema, and Workspace schema', () => {
    ...
    expect(Object.keys(CREATIVE_RUNTIME_SKILL_REGISTRY).sort()).toEqual([...CREATIVE_WORKER_KINDS].sort())

    for (const workerKind of CREATIVE_WORKER_KINDS) {
      const runtimeSkill = CREATIVE_RUNTIME_SKILL_REGISTRY[workerKind]
      const outputKind = CREATIVE_WORKER_OUTPUT_KIND[workerKind]
      const output = CREATIVE_OUTPUT_REGISTRY[outputKind]
      expect(runtimeSkill.outputKind, workerKind).toBe(outputKind)
      expect(output.workerKind, workerKind).toBe(workerKind)
      expect(runtimeSkill.skillIds, workerKind).toEqual(['creative-core', output.professionalSkillId])
