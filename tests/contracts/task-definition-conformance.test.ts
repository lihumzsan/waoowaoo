...
describe('TaskDefinition conformance', () => {
  it('registers every TaskType exactly once and drives queue/retry policy', () => {
    ...
    for (const taskType of taskTypes) {
      ...
      expect(['none', 'edit_bible', 'edit_style_preview', 'video_segment', 'chapter_render', 'final_video_render', 'music_score', 'ambient_sound', 'edit_script', 'edit_shot_execution_plan']).toContain(definition.terminalFailureProjector)
      expect(['none', 'edit_bible', 'edit_style_preview', 'video_segment', 'chapter_render', 'final_video_render', 'music_score', 'ambient_sound', 'edit_script', 'edit_shot_execution_plan']).toContain(definition.terminalCancelProjector)
    }
  })
  ...
  it('declares every current terminal failure projector without a default branch', () => {
    ...
    expect(TASK_DEFINITIONS.ambient_sound_plan.terminalFailureProjector).toBe('ambient_sound')
    expect(TASK_DEFINITIONS.ambient_sound_plan.terminalCancelProjector).toBe('ambient_sound')
