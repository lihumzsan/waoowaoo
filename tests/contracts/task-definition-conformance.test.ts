...
describe('TaskDefinition conformance', () => {
  it('registers every TaskType exactly once and drives queue/retry policy', () => {
    ...
    for (const taskType of taskTypes) {
      ...
      expect(['none', 'edit_bible', 'edit_style_preview', 'video_segment', 'chapter_render', 'final_video_render', 'music_score', 'ambient_sound', 'audio_design', 'edit_script', 'edit_shot_execution_plan']).toContain(definition.terminalFailureProjector)
      expect(['none', 'edit_bible', 'edit_style_preview', 'video_segment', 'chapter_render', 'final_video_render', 'music_score', 'ambient_sound', 'audio_design', 'edit_script', 'edit_shot_execution_plan']).toContain(definition.terminalCancelProjector)
    }
  })
  ...
  it('declares every current terminal failure projector without a default branch', () => {
    ...
    expect(TASK_DEFINITIONS.audio_design_plan.terminalFailureProjector).toBe('audio_design')
    expect(TASK_DEFINITIONS.audio_design_plan.terminalCancelProjector).toBe('audio_design')
    ...
    expect(TASK_DEFINITIONS.ambient_sound_generate.terminalFailureProjector).toBe('ambient_sound')
    expect(TASK_DEFINITIONS.ambient_sound_generate.terminalCancelProjector).toBe('ambient_sound')
