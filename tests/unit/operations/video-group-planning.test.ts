import {
  ...
  buildShots,
  buildSourceSnapshot,
  createVideoGenerationOperations,
  describe,
  expect,
  it,
  planVideoGroupTask,
  ...
  submitApprovedPlanMock,
  vi,
} from './video-group-planning.fixture'
...
describe('video group planning', () => {
  ...
  it('commits continuous video group records with shot numbers from the edit script', async () => {
    ...
    expect(plan).toBeDefined()
    if (plan) {
      submitApprovedPlanMock.mockResolvedValue(new Map(plan.tasks.map((task) => [task.id, {
    ...
    const result = plan
      ? await operation.commit?.(buildContext(), { episodeId: 'episode-1' }, plan)
      : null
