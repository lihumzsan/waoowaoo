...
describe('project agent prompt copy', () => {
  ...
  it('describes script ingestion as source-text only', () => {
    ...
    expect(zhDescription).toContain('过于稀疏的创意必须先走扩写前创作问诊')
    ...
    expect(enDescription).toContain('Sparse ideas must first use pre-expansion creative intake')
    ...
    expect(zhIntakeDescription).toContain('必须请求一次')
