...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('必须由你主动通过 tool use 发起')
    expect(prompt).toContain('choiceType="duration"')
    expect(prompt).toContain('choiceType="style_and_aspect_ratio"')
    ...
    expect(prompt).toContain('若没有 ready 的 editScreenplay，先按需用 request_edit_first_choice 确认时长，再调用 generate_edit_screenplay')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('When edit-first production needs a user choice, initiate it with tool use')
    expect(prompt).toContain('choiceType="duration"')
    expect(prompt).toContain('choiceType="style_and_aspect_ratio"')
