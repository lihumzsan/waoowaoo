...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('choiceType="next_step_confirmation"')
    expect(prompt).toContain('如果用户表示不满意、要求重做或调整候选图')
    expect(prompt).toContain('styleDirection')
    expect(prompt).toContain('后续每一个当前唯一下一步 edit-first operation 都必须先用卡片确认')
    expect(prompt).toContain('当 assistant 面板展示剪辑先行时长、剧本审核、视觉风格、画面比例或下一步确认卡时，必须等待用户点击或提交卡片')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('styleDirection')
    expect(prompt).toContain('every later immediate-next edit-first operation must be confirmed with a card before execution')
    expect(prompt).toContain('When the assistant panel displays a choice card for edit-first duration, screenplay review, visual style, aspect ratio, or next-step confirmation')
