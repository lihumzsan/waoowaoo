...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('失败处理与自动重试')
    expect(prompt).toContain('同一阶段连续失败达到 3 次时')

    expect(prompt).toContain('剪辑先行工作流')
    expect(prompt).toContain('源剧本摄入 -> Bible/章节切分 -> 用户审核 Bible')
    ...
    expect(prompt).toContain('剧本生成/修改是异步任务')
    expect(prompt).toContain('从上下文读取当前完整剧本，并在对话中逐字、完整输出')
    expect(prompt).toContain('立刻调用剧本修改工具；不要推迟到视觉风格阶段')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('Failure handling and automatic retry')
    expect(prompt).toContain('Only after the same stage fails three consecutive times')

    expect(prompt).toContain('Edit-first workflow')
    expect(prompt).toContain('source script ingestion -> bible/chapter split -> user bible review')
    ...
    expect(prompt).toContain('On tool failure, never fabricate bibles, edit tables, assets, storyboards, task progress, or substitute results')
    expect(prompt).toContain('bible-review outcome')
    expect(prompt).toContain('visual style')
    expect(prompt).toContain('asset-review outcome')
    expect(prompt).not.toContain('choiceType="next_step_confirmation"')

    expect(prompt).toContain('Generation/revision is async')
    expect(prompt).toContain('read the current full bible from context and output it verbatim and complete in chat')
    expect(prompt).toContain('call the bible revision tool now; do not defer to the visual style stage')
  ...
  it('describes script ingestion as source-text only', () => {
    ...
    expect(zhDescription).toContain('源剧本')
    expect(zhDescription).toContain('只传用户本轮提供的 text')
    expect(zhDescription).toContain('不要传 projectId、episodeId')
    expect(enDescription).toContain('source script')
    expect(enDescription).toContain('Pass only the text supplied by the user this turn')
  ...
  it('describes project context reads as concrete-detail only', () => {
    ...
    expect(enDescription).toContain('full bible text, historical generation results, failure details, active task details, or asset/storyboard/panel fields')
  ...
  it('localizes user-facing operation titles without exposing internal ids', () => {
    ...
    expect(localizeProjectAgentOperationTitle('ingest_script', 'zh')).toBe('摄入源剧本')
    expect(localizeProjectAgentOperationTitle('ingest_script', 'en')).toBe('Ingest script')
  ...
  it('describes style preview generation as regeneratable with user direction only', () => {
    ...
    expect(enDescription).toContain('Generate bible-based visual style preview images')
