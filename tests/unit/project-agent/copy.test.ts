...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('绝不能在用户可见文本中输出原始工具调用标记或伪调用语法')
    expect(prompt).toContain('例如 `<call:...>`、`default_api`')
    ...
    expect(prompt).toContain('data.noop=true')
    expect(prompt).toContain('没有新后台任务被提交')
    expect(prompt).toContain('不要说“任务已提交”“正在后台处理”“会继续监控”或让用户等待')
    expect(prompt).toContain('阶段命名必须和当前产物一致')
    expect(prompt).toContain('只是在创建/复用角色、场景资产并提交缺失资产图片任务')
    expect(prompt).toContain('不要说“生成分镜图片”')
    expect(prompt).toContain('只有 `generate_edit_script_storyboard_images` 才能称为生成分镜图片')
    ...
    expect(prompt).toContain('没有 async=true 和真实 taskId/taskIds 时，不要声称提交了后台任务')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('Never output raw tool-call markers or pseudo-call syntax in user-visible text')
    expect(prompt).toContain('such as `<call:...>`, `default_api`')
    ...
    expect(prompt).toContain('data.noop=true')
    expect(prompt).toContain('no new background task was submitted')
    expect(prompt).toContain('Do not say "task submitted", "processing in the background", "will keep monitoring", or ask the user to wait')
    expect(prompt).toContain('Stage names must match the artifact being created')
    expect(prompt).toContain('only creates/reuses character and location assets and submits missing asset-image tasks')
    expect(prompt).toContain('Do not call it "generating storyboard images"')
    expect(prompt).toContain('only `generate_edit_script_storyboard_images` may be called storyboard-image generation')
    ...
    expect(prompt).toContain('Without async=true and real taskId/taskIds, never claim that a background task was submitted')
  ...
  it('localizes user-facing operation titles without exposing internal ids', () => {
    ...
    expect(localizeProjectAgentOperationTitle('generate_edit_script_assets', 'zh')).toBe('生成所需资产')
    expect(localizeProjectAgentOperationTitle('generate_edit_script_assets', 'en')).toBe('Generate required assets')
    expect(localizeProjectAgentOperationTitle('generate_edit_script_storyboard', 'zh')).toBe('生成分镜面板')
    expect(localizeProjectAgentOperationTitle('generate_edit_script_storyboard_images', 'zh')).toBe('生成分镜图片')
