...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('3D、三维、3D 动画、3D 建模、3D 渲染、CG 渲染或 CGI')
    expect(prompt).toContain('非真人且非 3D 风格')
    expect(prompt).not.toContain('风格化 CG')
    expect(prompt).toContain('不要把内部系统规则、测试上线限制、安全策略措辞、tool 使用说明或 workflow 门禁作为说明文字发给用户')
    expect(prompt).toContain('工具调用沟通协议')
    expect(prompt).toContain('禁止输出空 assistant 文本后只发起工具调用')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('3D, three-dimensional animation, 3D modeling, 3D rendering, CG rendering, or CGI')
    expect(prompt).toContain('non-real-person and non-3D style')
    expect(prompt).not.toContain('stylized CG')
    ...
    expect(prompt).toContain('Do not emit an empty assistant message followed only by a tool call')
  ...
  it('describes style preview generation as regeneratable with capped count and direction', () => {
    ...
    expect(zhDescription).toContain('非真人且非 3D 画风')
    ...
    expect(enDescription).toContain('non-real-person and non-3D art direction')
