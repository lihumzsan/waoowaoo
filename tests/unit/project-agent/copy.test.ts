...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('只在对话中给出简短的规划摘要')
    expect(prompt).toContain('不要把完整 Bible、节拍表、事件台账、情绪曲线或章节全文重复发送到对话')
    expect(prompt).toContain('确认制作规划时，系统会依据确认卡中的精确计划自动提交视觉风格候选图任务')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('give only a concise planning summary in chat')
    expect(prompt).toContain('do not repeat the full Bible, beat sheet, event ledger, emotional curve, or chapter text in chat')
    expect(prompt).toContain('the system automatically submits the visual-style preview tasks from the exact plan shown on that confirmation card')
