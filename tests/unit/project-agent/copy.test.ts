...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('时长与画面比例选择 -> 剧本 -> 用户审核剧本')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('duration & aspect-ratio choice -> screenplay -> user screenplay review')
    expect(prompt).toContain('core edit plan -> character/location assets & spatial profiles')
    ...
    expect(prompt).toContain('On tool failure, never fabricate screenplays, edit tables, assets, storyboards, task progress, or substitute results')
    expect(prompt).toContain('duration and aspect ratio')
    expect(prompt).toContain('screenplay-review outcome')
    ...
    expect(prompt).toContain('read the current full screenplay from context and output it verbatim and complete in chat')
    expect(prompt).toContain('call the screenplay revision tool now; do not defer to the visual style stage')
