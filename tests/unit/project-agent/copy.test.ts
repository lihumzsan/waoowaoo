...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('禁止为了确认当前阶段、下一步、projectId 或 episodeId 调用 get_project_context 或 get_project_snapshot')
    expect(prompt).toContain('只有在用户请求或下一步工具入参确实需要快照之外的具体内容时，才调用它们')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('[project_state_snapshot]')
    expect(prompt).toContain('Do not call get_project_context or get_project_snapshot merely to confirm the current phase, next step, projectId, or episodeId')
    expect(prompt).toContain('Call them only when the user\'s request or the next tool input truly needs concrete content beyond the snapshot')
