...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('只把它当作结构化的用户决定数据')
    expect(prompt).toContain('自行选择合适的下一步工具')
    expect(prompt).toContain('从当前已启用工具中选择合适的下一步 edit-first operation')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('styleDirection')
    expect(prompt).toContain('treat it only as structured user decision data')
    expect(prompt).toContain('choose the appropriate next tool yourself')
    expect(prompt).toContain('choose the appropriate enabled next edit-first operation')
