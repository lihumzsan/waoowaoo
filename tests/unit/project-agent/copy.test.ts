...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('工具调用前说明必须是一段简短文字，最多两句')
    expect(prompt).toContain('禁止重复同一句，禁止多次重述同一张选择卡说明')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('The pre-tool explanation must be one concise paragraph with at most two short sentences')
    expect(prompt).toContain('Never repeat the same sentence, never restate the same card instruction multiple times')
