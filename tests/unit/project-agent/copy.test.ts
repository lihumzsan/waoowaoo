...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('当 assistant 面板展示剪辑先行时长、剧本审核、视觉风格、资产审核或画面比例选择卡时，必须等待用户点击或提交卡片')
    expect(prompt).toContain('禁止跳步、批量推进多个未来阶段、执行后置阶段 operation')
    expect(prompt).toContain('generate_edit_screenplay 成功后，必须读取 tool 结果里的 data.screenplayText')
    ...
    expect(prompt).toContain('revise_edit_screenplay 成功后，必须读取 tool 结果里的 data.screenplayText')
    expect(prompt).toContain('把修改后的完整剧本逐字输出给用户')
    expect(prompt).toContain('剧本生成或修改是例外：必须先输出返回的完整 screenplayText')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('When the assistant panel displays a choice card for edit-first duration, screenplay review, visual style, asset review, or aspect ratio')
    expect(prompt).toContain('Assistant permission mode: auto')
    expect(prompt).toContain('After generate_edit_screenplay succeeds, read data.screenplayText from the tool result')
    ...
    expect(prompt).toContain('After revise_edit_screenplay succeeds, read data.screenplayText from the tool result')
    expect(prompt).toContain('present that exact full revised screenplay content to the user')
    expect(prompt).toContain('The exception is screenplay generation or revision: output the full returned screenplayText')
  ...
  it('describes screenplay generation as requiring structured duration and aspect ratio fields', () => {
    ...
    expect(zhDescription).toContain('成功后必须读取返回结果中的 screenplayText')
    ...
    expect(enDescription).toContain('After success, read screenplayText from the tool result')
    expect(enDescription).toContain('echo the complete text to the user in chat')
  })
  ...
  it('describes screenplay revision as review-stage only with structured fields', () => {
    ...
    expect(enDescription).toContain('After success, read screenplayText from the tool result')
    expect(enDescription).toContain('echo the complete revised screenplay to the user in chat')
  })
