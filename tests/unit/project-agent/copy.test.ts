...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('可直接扩写的详细创作简报')
    expect(prompt).toContain('只有满足这些基本条件，才可以直接调用 `ingest_script`')
    expect(prompt).toContain('只要输入不满足完整剧本或详细创作简报的基本条件，就必须先调用扩写前创作问诊选择工具')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('directly expandable detailed creative brief')
    expect(prompt).toContain('Only when those basics are present may you call `ingest_script`')
    expect(prompt).toContain('if the input does not meet the basic conditions for a complete script or detailed creative brief, you must first call the pre-expansion creative intake choice tool')
  ...
  it('describes script ingestion as source-text only', () => {
    ...
    expect(zhIntakeDescription).toContain('不是完整可拍剧本')
    expect(zhIntakeDescription).toContain('详细到不需要追问')
    expect(enIntakeDescription).toContain('pre-expansion creative intake')
    expect(enIntakeDescription).toContain('not provided a complete filmable script')
    expect(enIntakeDescription).toContain('detailed enough that no follow-up is needed')
  })
