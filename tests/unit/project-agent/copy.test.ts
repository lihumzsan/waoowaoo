...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('源剧本摄入/扩写 -> 用户确认剧本（仅 prompt 扩写时）-> 制作规划')
    ...
    expect(prompt).toContain('立刻调用制作规划修改工具；不要推迟到视觉风格阶段')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('source script ingestion/expansion -> user script confirmation (prompt-expansion path only) -> production plan')
    ...
    expect(prompt).toContain('production-plan confirmation outcome')
    expect(prompt).toContain('visual style')
    expect(prompt).toContain('asset-review outcome')
    expect(prompt).not.toContain('choiceType="next_step_confirmation"')

    expect(prompt).toContain('Production-plan generation or revision is async')
    expect(prompt).toContain('choiceType=script_intake')
    expect(prompt).toContain('normalizedBrief')
    expect(prompt).toContain('in the vast majority of cases you must first call the pre-expansion creative intake choice tool')
    expect(prompt).toContain('era and setting, protagonist identity and character motivation, core conflict')
    expect(prompt).toContain('unless the user directly provided a complete, filmable script, you must first call the pre-expansion creative intake choice tool')
    expect(prompt).toContain('read the current full production plan, chapter split, and key planning summary from context and output the confirmable content completely in chat')
    expect(prompt).toContain('call the production-plan revision tool now; do not defer to the visual style stage')
    ...
    expect(prompt).toContain('Script confirmation:')
    expect(prompt).toContain('generate_bible_from_script')
  ...
  it('describes script ingestion as source-text only', () => {
    ...
    expect(zhDescription).toContain('完整剧本')
    expect(zhDescription).toContain('sourceKind=paste')
    expect(zhDescription).toContain('sourceKind=prompt_generated_outline')
    expect(zhDescription).toContain('凡是缺少剧本基本条件')
    expect(zhDescription).toContain('必须先用 request_script_intake_choice 做问诊')
    ...
    expect(enDescription).toContain('complete script')
    expect(enDescription).toContain('sourceKind=paste')
    expect(enDescription).toContain('sourceKind=prompt_generated_outline')
    expect(enDescription).toContain('lacks basic script conditions')
    expect(enDescription).toContain('must run request_script_intake_choice first')
    ...
    expect(zhIntakeDescription).toContain('缺少剧本的基础结构信息')
    expect(zhIntakeDescription).toContain('除非用户直接给出了完整可拍剧本')
  ...
  it('localizes user-facing operation titles without exposing internal ids', () => {
    ...
    expect(localizeProjectAgentOperationTitle('ingest_script', 'zh')).toBe('创作剧本')
    expect(localizeProjectAgentOperationTitle('ingest_script', 'en')).toBe('Create script')
    expect(localizeProjectAgentOperationTitle('revise_script', 'zh')).toBe('修改剧本')
    expect(localizeProjectAgentOperationTitle('revise_script', 'en')).toBe('Revise script')
    expect(localizeProjectAgentOperationTitle('generate_bible_from_script', 'zh')).toBe('生成制作规划')
    expect(localizeProjectAgentOperationTitle('generate_bible_from_script', 'en')).toBe('Generate production plan')
    expect(localizeProjectAgentOperationTitle('request_edit_script_review_choice', 'zh')).toBe('确认剧本')
    expect(localizeProjectAgentOperationTitle('request_edit_script_review_choice', 'en')).toBe('Confirm script')
  ...
  it('describes bible revision as a user-note-driven overwrite', () => {
    ...
    expect(zhDescription).toContain('修改当前的制作规划')
    expect(zhDescription).toContain('根据用户在制作规划确认中的修改意见')
    expect(zhDescription).not.toContain('projectId')
