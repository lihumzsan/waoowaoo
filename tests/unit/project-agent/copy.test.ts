...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('发起下一步工具调用前，必须在同一轮 assistant 回复里先用简短的用户可见自然语言说明')
    expect(prompt).toContain('禁止提内部 operation 名、tool id、workflow 门禁或审批实现细节')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('Before initiating a next-step tool call, write a brief user-facing explanation')
    expect(prompt).toContain('Do not mention internal operation names, tool ids, workflow gates, or approval implementation details')
