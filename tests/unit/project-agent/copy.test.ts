...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('源剧本摄入/扩写 -> 用户确认剧本（仅 prompt 扩写时）-> 剧集规划')
    ...
    expect(prompt).toContain('从上下文读取当前完整剧集规划、章节切分和关键规划摘要，并在对话中完整输出可确认内容')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('source script ingestion/expansion -> user script confirmation (prompt-expansion path only) -> episode plan')
    ...
    expect(prompt).toContain('read the current full episode plan, chapter split, and key planning summary from context and output the confirmable content completely in chat')
  ...
  it('describes bible revision as a user-note-driven overwrite', () => {
    ...
    expect(zhDescription).toContain('修改当前的剧集规划')
    expect(zhDescription).toContain('根据用户在剧集规划确认中的修改意见')
    expect(zhDescription).not.toContain('projectId')
