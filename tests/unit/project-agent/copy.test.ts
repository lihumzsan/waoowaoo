...
describe('project agent prompt copy', () => {
  it('builds the Chinese project-agent system prompt with staged workflow guardrails', () => {
    ...
    expect(prompt).toContain('Bible/章节切分审核')
    expect(prompt).toContain('视觉风格')
    expect(prompt).toContain('资产审核')
    expect(prompt).not.toContain('choiceType="next_step_confirmation"')

    expect(prompt).toContain('Bible/章节切分生成或修改是异步任务')
    expect(prompt).toContain('从上下文读取当前完整 Bible 和章节切分，并在对话中完整输出可审核内容')
    expect(prompt).toContain('立刻调用 Bible 修改工具；不要推迟到视觉风格阶段')
  ...
  it('builds the English project-agent system prompt with matching workflow guardrails', () => {
    ...
    expect(prompt).toContain('Bible/chapter-split review outcome')
    expect(prompt).toContain('visual style')
    expect(prompt).toContain('asset-review outcome')
    expect(prompt).not.toContain('choiceType="next_step_confirmation"')

    expect(prompt).toContain('Bible/chapter-split generation or revision is async')
    expect(prompt).toContain('read the current full Bible and chapter split from context and output the reviewable content completely in chat')
