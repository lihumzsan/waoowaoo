...
describe('project agent prompt copy', () => {
  it('uses direct operation rules instead of fixed workflow or skill-gateway rules', () => {
    ...
    expect(prompt).toContain('禁止为了确认当前阶段、下一步、projectId 或 episodeId 调用 get_project_context')
    expect(prompt).toContain('只有在用户请求或下一步工具入参确实需要快照之外的具体内容时，才调用 get_project_context')
  ...
  it('adds the same next-step gate to the English prompt', () => {
    ...
    expect(prompt).toContain('Do not call get_project_context merely to confirm the current phase, next step, projectId, or episodeId')
    expect(prompt).toContain('Call get_project_context or get_project_snapshot only when the user\'s request or the next tool input truly needs concrete content beyond the snapshot')
  ...
  it('describes project context reads as concrete-detail only', () => {
    ...
    expect(zhDescription).toContain('仅在本轮注入的 project_state_snapshot 不足以回答用户请求或补齐下一步工具入参时')
    expect(zhDescription).toContain('完整剧本、历史生成结果、活动任务详情、资产/分镜/面板字段')
    expect(zhDescription).toContain('禁止仅为了确认当前阶段、下一步、projectId 或 episodeId 调用')
    expect(enDescription).toContain('only when the injected project_state_snapshot is insufficient')
    expect(enDescription).toContain('full screenplay text, historical generation results, active task details, or asset/storyboard/panel fields')
    expect(enDescription).toContain('Do not call merely to confirm the current phase, next step, projectId, or episodeId')
  })
