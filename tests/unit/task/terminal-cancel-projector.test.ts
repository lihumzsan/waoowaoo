...
const projector = vi.hoisted(() => vi.fn(async () => undefined))

vi.mock('@/lib/prisma', () => ({
  prisma: { $transaction: vi.fn(async (run: (client: typeof tx) => Promise<unknown>) => await run(tx)) },
}))
vi.mock('@/lib/billing', () => ({
...
describe('Task terminal cancel projector', () => {
  ...
  it.each(['queued', 'processing'])('projects %s cancellation before committing the Task terminal row', async (status) => {
    state.status = status
    ...
    expect(projector.mock.invocationCallOrder[0]).toBeLessThan(tx.task.updateMany.mock.invocationCallOrder[0] ?? Infinity)
