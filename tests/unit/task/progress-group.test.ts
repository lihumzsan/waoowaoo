...
describe('task progress group', () => {
  it('builds a stable operation progress group only when operation metadata is complete', () => {
    ...
    })).toBe('operation:generate_edit_script_storyboard_images:request-1')
  ...
  it('writes and reads progress group id inside task ui payload without dropping existing fields', () => {
    ...
    }, 'operation:generate_edit_script_storyboard_images:request-1')
    ...
    expect(readTaskPayloadProgressGroupId(payload)).toBe('operation:generate_edit_script_storyboard_images:request-1')
  })
})
