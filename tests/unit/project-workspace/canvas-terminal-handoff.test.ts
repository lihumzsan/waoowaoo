 ...
 * Rejects: removing a canonical node or its last preview after Task completion but before the formal Query arrives.
 * Production entries: buildWorkspaceNodeCanvasProjection and resolveWorkspaceCanvasNodeData.
 * Oracle: durable identity keeps the node materialized and completed handoff content remains until persisted content succeeds.
...
function sourceNode(): WorkspaceCanvasFlowNode {
  return {
