...
describe('workspace canvas layout runtime contract', () => {
  ...
  it('keeps expanded layout dimensions during collapse exit motion', () => {
    ...
    expect(canvas).toContain('readonly layoutExpanded: boolean')
    ...
    expect(canvas).toContain('expanded: false,')
    expect(canvas).toContain('layoutExpanded: true,')
    expect(canvas).toContain('expanded: true,')
    expect(canvas).toContain('const layoutExpanded = expanded || disclosureOverride?.layoutExpanded === true')
