...
| AI turn stops after a successful tool call | baseline: successful `confirm_bible` followed by `PROJECT_AGENT_WORKFLOW_CONTINUATION_MISSING: generate_edit_style_previews`; 后续真实项目复现 `PROJECT_AGENT_AI_TURN_PROTOCOL_REQUIRED` | prompt 仍描述已删除的自动视觉候选提交路径，runtime 又把 capability 解释为 obligation；随后删除硬失败仍留下 Choice→AI 确认中转和旧状态快照 | deterministic model 按私有优先级继续，旧 model-stop 场景未到达 Bible 确认 | `nextAction` 仍不是服务端义务；确定性用户确认改为 Choice 事务内 Operation，model-stop 在刷新后的首个回合停止并验证确认保留、零后续副作用；正常 mainline 独立证明 AI 可从新状态继续 |
