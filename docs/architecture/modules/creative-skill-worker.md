...
- **CS-02 — V1 Skill 保持单层且职责分离。** 每个已注册 Skill 只有同目录下的 `SKILL.zh.md` 与 `SKILL.en.md`；V1 不建立 `references/`、固定专业角色或递归知识树。`story-development` 负责创作剧本文本；`screenplay-canonicalization` 只在显式需要结构化分析时，把生成文本或精确来源 Revision 规范化为实体与场景引用，不负责保存或确认用户原文；`style-development` 独占全局视觉语言和 Style Bible，`asset-development` 在同一个 Task 内完成已登记实体的资产事实提取、外观设计与最终媒体 Prompt。是否需要 Style Bible、Bible、Chapter、连续性分析、canonical screenplay 或资产由 Primary 依据当前目标与事实判断；提供这些输入时 Worker 只能消费精确 Revision，不能反向改写。中英文 Skill 必须表达同一业务规则集合。
...
- **CS-06 — 输出契约穷尽。** Worker output kind 与 strict schema 只由 `creativeWorkOutputRegistry` 声明。当前集合为 `canonical_screenplay`、`edit_bible_bundle`、`chapter_plan`、`continuity_analysis`、`style_bible`、`asset_manifest`、`video_prompt_set`、`music_direction` 与 `creative_review`。显式请求的 `canonical_screenplay` 由确定性编译器补齐稳定 entity/scene identity 后物化为结构化文字 Resource Revision；从零生成与精确来源文本在这个分析输出中共用同一契约，只在 `source` 保留来源，但用户原文的直接保存不以此 Task 为前置。`asset_manifest` 必须精确引用一个 canonical screenplay Revision 与项目当前采用的 Style Revision，并对登记实体恰好覆盖一次；提取、设计和 Prompt 是一次 Worker 判断，不存在单独 extraction output。`style_bible` 可返回 finalized 或任意 2–12 个完整文字候选，只有显式 `adopt_style_bible` 才写 `project.style_bible` Binding，默认不生成预览图。未知、缺失、错 kind 或超预算输出必须原地失败。
...
- **CS-09 — Subagent UI 只按 origin Run 投影 Task。** SessionState 先按当前 `assistantId + scopeRef` 读取权威 Assistant Runs，再只接收 `payload.origin.runId` 属于这些 Runs 且声明 canonical `creative_work_v4` protocol 的 `creative_work` Task，以 `Task.status` 与 lifecycle projection 构造运行中和终态 View。Task 的 `episodeId` 表达输出 Resource scope，不能用来裁剪 Subagent UI；因此 Episode Run 发起的项目级 screenplay/style Task 仍属于该 Run 的 Subagent。同一 Task 不能同时显示为普通运行卡与 Subagent。Primary 消息锚点只提供 status、output kind、Subagent 标识和详情入口，不内联 reasoning、Tool 或结果；选中详情默认展开按事件顺序投影的公开 reasoning/Tool 轨迹，终态 summary 保持可读，完整 `Task.result` JSON 由同 scope reader 读取并默认折叠。刷新读取 durable snapshot，实时 reasoning 只作为带 identity/seq 的 overlay；历史 message、Tool 卡、文案和 DOM 不得反向推断状态。不兼容 protocol 一次性切换，不提供 v3/v4 双 parser。
...
- **CS-12 — 剧本与风格没有前后置耦合。** `canonical_screenplay` 不读取或写入 Style，`style_bible` 不要求已有剧本；Primary 可以按用户目标先生成任意一方。只有正式 `asset_manifest` 同时需要规范剧本和当前 adopted Style，缺任一项必须显式失败，但不得自动生成、自动采用或启动下一步。

## 权威入口

- Skill identity、目录与读取：`src/lib/creative-skills/registry.ts`、`uri.ts`、`loader.ts`。
- 单层 Skill 内容：`src/lib/creative-skills/skills/*/SKILL.zh.md` 与 `SKILL.en.md`；剧本规范化、风格与资产分别由 `screenplay-canonicalization`、`style-development`、`asset-development` 独占。
...
- Canonical screenplay 编译与 Asset Manifest 精确覆盖：`src/lib/canonical-screenplay/**`；两者终态仍只由 `src/lib/creative-resource/creative-work-materialization.ts` 物化。
...
- 目标分支曾把从零写剧本、剧本登记、风格与资产串成固定 00–07S 管线，并允许 Worker 给出最终实体 key；首次自由化虽然删除了确认状态，却仍要求用户粘贴的完整剧本先经过 canonicalization 才能作为剧本存在，保留了一个换名后的固定前置。当前用户原文先由既有 `create_text.current_user_text` 精确保存；只有消费者明确需要实体/场景结构时才独立请求 `canonical_screenplay`，稳定 ID 只由服务端编译，风格与剧本顺序独立。`asset-development` 只消费两份精确事实并一次输出 manifest。真实模型对别名归一和实体覆盖的质量仍是生成复验盲区，确定性 identity 与 exact-once 覆盖由编译/物化层 fail closed。
...
- Subagent UI 初版按 `Task.episodeId` 查询 Task，而 Project 级 `canonical_screenplay/style_bible` Task 的该字段为 null；即使 `payload.origin.runId` 明确属于当前 Episode Run，刷新和终态 UI 仍会把它排除。现以 Assistant Run 的 `assistantId + scopeRef` 选择 origin run，再按 immutable `payload.origin.runId` 投影 Task；Task episode 只保留输出 Resource scope 语义。Golden 对项目级 screenplay Task 的可见 tab 是反证防线。
- Subagent reasoning 与 Tool 轨迹补齐后，详情用的 Disclosure 又被直接复用进 Primary 消息卡，summary 也重复内联；结果是每个失败/完成锚点都展开第二行轨迹入口，而详情反而在终态自动折叠轨迹、直接铺开完整 strict JSON，并用多条横线与 reasoning 竖线切割同一阅读流。现 Primary 锚点只保留状态、产物名、Subagent 标识和详情入口；详情独占默认展开的公开轨迹与 summary，完整 JSON 默认折叠，Primary/详情 reasoning 均不再用分隔线表达层级。Task/result/stream owner 均未改变；现有 stream Logic 不承担视觉层级 oracle，authenticated workspace 仍是最终视觉复验边界。
...
9. 剧本规范化、风格与资产是否分别归 `screenplay-canonicalization` / `style-development` / `asset-development`，且没有固定先后、独立 extraction output 或 Worker 自定义实体 key？
