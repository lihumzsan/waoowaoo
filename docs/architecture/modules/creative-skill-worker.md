...
本阶段把唯一 `delegation` 联合与服务端 `productionContext` 加入带显式 `creative_work_v2` identity 的新 `creative_work` Task payload，属于不兼容的运行协议切换。Session 只投影该 protocol，历史终态旧协议 Task 不进入新 Subagent View；部署前仍必须确认旧版本创建的 queued/processing `creative_work` Task 已经排空，避免新 Worker 消费旧 payload。不得为旧 payload 增加旧分支 parser、默认画幅或默认时长。部署后只有新委派入口可以写入该 protocol。
