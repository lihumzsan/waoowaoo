...
- **CS-09 — Subagent UI 只投影 Task。** SessionState 从当前 scope 内 queued/processing 的 `creative_work` Task 及其 lifecycle projection 构造运行中标签、数量和 Skill 读取详情；刷新与 SSE 更新均重读该最终 View。完成、失败或取消后 Task 离开 active 集合，标签自动消失。传输增量、历史 message、Tool 卡、文案和 DOM 不得反向推断 Subagent 状态。
