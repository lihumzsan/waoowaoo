...
- **CS-02 — V1 Skill 保持单层且职责分离。** 每个已注册 Skill 只有同目录下的 `SKILL.zh.md` 与 `SKILL.en.md`；V1 不建立 `references/`、固定专业角色或递归知识树。`story-development` 只负责创作或修改剧本文本；`style-development` 独占全局视觉语言和 Style Bible；`asset-development` 在同一个 `asset_manifest` Task 内独占生产资产筛选、事实归一、外观设计与最终媒体 Prompt。是否需要 Style Bible、Story Canon、Chapter、连续性、screenplay 或资产由 Primary 依据当前目标与事实判断；提供这些输入时 Worker 只能消费精确 Revision，不能反向改写。中英文 Skill 必须表达同一业务规则集合。
...
- **CS-06 — 输出契约穷尽。** Worker output kind 与 strict schema 只由 `creativeWorkOutputRegistry` 声明。当前集合为 `screenplay`、`story_canon`、`chapter_plan`、`continuity_analysis`、`style_bible`、`asset_manifest`、`video_prompt_set`、`music_direction` 与 `creative_review`。`screenplay` 只物化文本与写作元信息，不含生产资产 registry、scene ranges 或数据库 identity；用户原文直接保存不以 Worker Task 为前置。`asset_manifest` 必须精确引用一个 screenplay Revision 与项目当前采用的 Style Revision，Worker 一次完成筛选、设计和 Prompt，并以剧本逐字 excerpt 证明每项来源；服务端只编译稳定 `manifestAssetId`、拒绝名称/别名歧义和无来源条目，不存在单独 extraction output 或 screenplay exact-coverage 前置。没有实体通过全部生产门槛时允许显式空 `assets` 并说明原因，禁止为满足非空约束制造多余资产。`style_bible` 可返回 finalized 或任意 2–12 个完整文字候选，只有显式 `adopt_style_bible` 才写 `project.style_bible` Binding，默认不生成预览图。未知、缺失、错 kind 或超预算输出必须原地失败。
...
- **CS-09 — Subagent UI 只按 origin Run 投影 Task。** SessionState 先按当前 `assistantId + scopeRef` 读取权威 Assistant Runs，再只接收属于 `SessionState.subagents` 权威 taskId 集且声明 canonical `creative_work_v5` protocol 的 `creative_work` Task，以 `Task.status` 与 lifecycle projection 构造运行中和终态 View；未知 lifecycle 最多触发一次 SessionState 刷新，服务端 origin Run projector 决定是否纳入。Task 的 `episodeId` 只表达输出 Resource scope，不能用来裁剪 Subagent UI；因此 Episode Run 发起的项目级 screenplay/style Task 仍属于该 Run 的 Subagent。同一 Task 不能同时显示为普通运行卡与 Subagent。Primary 消息锚点只提供 status、output kind、Subagent 标识和详情入口，不内联 reasoning、Tool 或结果；选中详情默认展开按事件顺序投影的公开 reasoning/Tool 轨迹，终态 summary 保持可读，完整 `Task.result` JSON 由同 scope reader 读取并默认折叠。刷新读取 durable snapshot，实时 reasoning 只作为带 identity/seq 的 overlay；seq gap 时只刷新一次、删除不完整 overlay 并抑制该 reasoning identity 的后续 delta，直到新 identity 或终态快照。挂载或切换先立即显示完整 canonical 快照，只有之后新增字符播放动画。历史 message、Tool 卡、文案和 DOM 不得反向推断状态。部署前必须排空 v4 queued/processing Task 与 Wait；不提供 v4/v5 双 parser。
- **CS-10 — Story Canon、Chapter 与连续性是独立能力。** `story_canon`、`continuity_analysis` 与 Chapter 计划分别是可独立请求、独立采用、独立版本化的事实；采用 Story Canon 不创建 Chapter，创建 Chapter 不自动委派 Worker，任何 Task 终态都只恢复 Primary。`compileCreativeChapterContext` 只在 Primary 显式以 Chapters 并行委派时，为一个 Chapter 从精确剧本/Story Canon/Chapter/Style/asset revisions 纯派生最小上下文；它不写事实、不创建 Task、不决定顺序。系统不持久化 WorkerGroup。
...
- **CS-12 — 剧本与风格没有前后置耦合。** `screenplay` 不读取或写入 Style，`style_bible` 不要求已有剧本；Primary 可以按用户目标先生成任意一方。只有正式 `asset_manifest` 同时需要精确 screenplay 和当前 adopted Style，缺任一项必须显式失败，但不得自动生成、自动采用或启动下一步。

## 权威入口

- Skill identity、目录与读取：`src/lib/creative-skills/registry.ts`、`uri.ts`、`loader.ts`。
- 单层 Skill 内容：`src/lib/creative-skills/skills/*/SKILL.zh.md` 与 `SKILL.en.md`；剧本创作、风格与资产分别由 `story-development`、`style-development`、`asset-development` 独占。
...
Primary 对每个目标评估独立工作单元、共享 canon、跨单元状态、上下文大小、并行恢复价值和用户意图。总时长大于 180 秒只是应认真评估全局连续性与 Chapter 的强信号，绝不是代码分支；短作品也可因多条并行叙事需要连续性，长作品也可在单一连续上下文中完成。Story Canon、Style Bible、Chapter、连续性分析或任一 Subagent Task 都不自动调用下一步。
...
- 目标分支曾把从零写剧本、剧本登记、风格与资产串成固定 00–07S 管线，并允许 Worker 给出最终实体 key；首次自由化虽然删除了确认状态，却仍要求用户粘贴的完整剧本先经过 canonicalization 才能作为剧本存在，保留了一个换名后的固定前置。当前用户原文由既有 `create_text.current_user_text` 精确保存为 screenplay，创作剧本只由 `story-development`，风格与剧本顺序独立；`asset-development` 消费精确 screenplay 与 adopted Style 并在一次 Task 内拥有筛选、设计和 Prompt。真实模型对别名归一和一次性关键空间的筛选质量仍是生成复验盲区，确定性 manifest identity、歧义与来源证据由物化层 fail closed。
...
- 一分钟视频真实请求曾由 Primary 在委派前硬编码“6 段、每段 10 秒”；首次防线又以 15/180 秒三个区间写出唯一 Playbook，并把 Style Bible/Chapter 变成固定前置，仍然是换名后的串行架构。当前只保留媒体 provider 的真实单次时长能力约束；Primary 依据独立工作单元、共享 canon、上下文和恢复价值判断是否使用 Story Canon/Chapter/连续性分析。`>180s` 只是强提醒，不能触发代码分支或 WorkerGroup。
...
- Subagent UI 初版按 `Task.episodeId` 查询 Task，而 Project 级 `screenplay/style_bible` Task 的该字段为 null；即使 `payload.origin.runId` 明确属于当前 Episode Run，刷新和终态 UI 仍会把它排除。现由服务端以 Assistant Run 的 `assistantId + scopeRef` 和 immutable `payload.origin.runId` 投影 SessionState.subagents，客户端 stream 只接受其中 taskId；Task episode 只保留输出 Resource scope 语义。Golden 对项目级 screenplay Task 的可见 tab 是反证防线。
...
9. 剧本创作、风格与资产是否分别归 `story-development` / `style-development` / `asset-development`，且 screenplay 没有生产资产登记、没有固定先后、独立 extraction output 或 Worker 自定义数据库 key？
