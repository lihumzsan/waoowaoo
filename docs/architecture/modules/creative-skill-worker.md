...
- **CS-02 — V1 Skill 保持单层且职责分离。** 每个已注册 Skill 只有同目录下的 `SKILL.zh.md` 与 `SKILL.en.md`；V1 不建立 `references/`、固定专业角色或递归知识树。`story-development` 负责创作剧本文本，`screenplay-canonicalization` 负责把生成、粘贴或导入文本统一规范化为实体与场景引用，`style-development` 独占全局视觉语言和 Style Bible，`asset-development` 在同一个 Task 内完成已登记实体的资产事实提取、外观设计与最终媒体 Prompt。是否需要 Style Bible、Bible、Chapter、连续性分析或资产由 Primary 依据当前目标与事实判断；提供这些输入时 Worker 只能消费精确 Revision，不能反向改写。中英文 Skill 必须表达同一业务规则集合。
...
- **CS-06 — 输出契约穷尽。** Worker output kind 与 strict schema 只由 `creativeWorkOutputRegistry` 声明。当前集合为 `canonical_screenplay`、`edit_bible_bundle`、`chapter_plan`、`continuity_analysis`、`style_bible`、`asset_manifest`、`video_prompt_set`、`music_direction` 与 `creative_review`。成功 `canonical_screenplay` 由确定性编译器补齐稳定 entity/scene identity 后物化为结构化文字 Resource Revision；生成、粘贴与导入共用这一份契约，只在 `source` 保留来源。`asset_manifest` 必须精确引用一个 canonical screenplay Revision 与项目当前采用的 Style Revision，并对登记实体恰好覆盖一次；提取、设计和 Prompt 是一次 Worker 判断，不存在单独 extraction output。`style_bible` 可返回 finalized 或任意 2–12 个完整文字候选，只有显式 `adopt_style_bible` 才写 `project.style_bible` Binding，默认不生成预览图。未知、缺失、错 kind 或超预算输出必须原地失败。
...
- 目标分支曾把从零写剧本、剧本登记、风格与资产串成固定 00–07S 管线，并允许 Worker 给出最终实体 key；这会让用户粘贴剧本走另一条提取路径，也让剧本/风格顺序成为工作流状态。当前统一为同一个 `canonical_screenplay` 契约，稳定 ID 只由服务端编译，风格与剧本顺序独立；`asset-development` 只消费两份精确事实并一次输出 manifest。真实模型对别名归一和实体覆盖的质量仍是生成复验盲区，确定性 identity 与 exact-once 覆盖由编译/物化层 fail closed。
