...
- **CS-06A — 导演、分段与视频设计是同一输出。** `video_prompt_set` 必须在一个 Subagent 结果内同时给出全局导演意图、权威画幅、整部/本章总时长、每个生成段的结构化导演时间线和可直接交给视频模型的 `finalPrompt`。Primary 只允许传总时长，严禁预先指定段数或逐段时长；服务端从固定视频能力注入允许时长、最小/最大时长与项目画幅，Worker 同时读取 `director-core + video-direction`，先导演再把连续内容尽量装入最大允许时长（当前产品上限 15 秒）。运行时穷尽验证画幅、允许时长、时间线连续覆盖和总和相等。不得把导演、分段和模型表达拆成隐藏串行 Task，也不得把导演决定只藏在自由文本 Prompt 中。
...
本阶段把服务端 `productionContext` 加入所有新 `creative_work` Task payload，属于不兼容的运行协议切换。部署前必须确认旧版本创建的 queued/processing `creative_work` Task 已经排空；不得为旧 payload 增加默认画幅、默认时长或兼容 parser。部署后只有新委派入口可以写入该 payload。
...
- finalized Style Bible 采用与唯一 Resource writer：`src/lib/operations/domains/assistant/creative-style-ops.ts`；它只读取 completed `creative_work` Task，拒绝 candidates，并追加不可变 `project.style_bible` revision。
...
主 Agent 按总时长选择配方：不超过 15 秒可单次生成；15–180 秒使用确认剧本 → finalized Style Bible → 重复身份所需资产 → 一个整片 `video_prompt_set` → 并行片段与合成；只有大于 180 秒才建立正式 Source/全局 Bible、使用唯一 splitter 与 `chapter_batch`。Bible、Style Bible、Chapter 或 Subagent Task 任一步都不自动调用下一步；依赖来自主 Agent 的显式 plan 与精确输入引用，而不是隐藏工作流。
