...
- **DE-03 — Coordinator 必须极薄。** 只允许 command admission、active Turn 互斥、
  waiting Approval、pending Choice Offer、pending follow-up FIFO、cancel/clear 与 model
  Activity 调度。
...
- **DE-09 — 只提交完整模型历史。** UI 流式增量可丢；只有完整且通过 SDK Session owner
  ...
  Provider、Billing 和 Resource 事实不会回滚，也不得伪造为未发生。
...
- **DE-12 — ToolEffect 只防同 Turn 技术重放。** canonical identity 为
  ...
  `InterruptedTurnEffectDigest` 与 Operation/Task/Provider 的业务幂等。
...
- **DE-16 — 容量释放与 Agent 通知解耦。** Task terminal 业务事务提交后，
  ...
  follow-up 通知可无限重试，但不得继续占用 Provider 容量。Scheduler 只保存
...
- **DE-21 — 协议升级不双读。** 正式环境必须使用不可变且非`local`的Worker build ID、
  启用Worker Deployment Versioning，并以`PINNED`作为默认versioning behavior；缺少任一
  条件启动即失败。Workflow变更遵守Deployment/patch纪律；不兼容的Workflow、SDK
  RunState、Task payload或schema发布必须先排空旧实例或明确阻止部署。禁止“先试新格式、
  失败再猜旧格式”。
...
- 部署preflight曾因开发/生产profile边界错误阻止正常开发；修正后新增Temporal时又一度
  允许正式环境沿用`local` build ID或关闭Versioning，属于同一“开发默认值不得进入生产”
  不变量的新实例漏接。当前production config与preflight共同强制immutable build、
  Versioning和PINNED，真实滚动发布/旧Workflow排空仍需目标环境复验。
