...
- **PS-09 — 认证防线与部署配置失败关闭。** 登录/注册限流只在 `TRUSTED_PROXY_HOPS` 明确声明后从右侧可信代理链解析客户端 IP；无法验证来源使用共享桶，Redis 不可用时拒绝认证尝试。cloud preflight 必须拒绝缺失、弱密钥、非 HTTPS 公网地址和未知代理拓扑；Compose 不得提供可用的默认密码或把基础设施默认绑定到公网。
