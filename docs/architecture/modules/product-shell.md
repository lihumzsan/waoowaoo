...
- **PS-11 — 认证入口与账号初始化唯一。** 产品只有一个登录/注册页面；登录只验证已存在身份，注册只
  ...
  保存，页面倒计时不承担安全或终态正确性。
- **PS-12 — 对象存储是唯一内网基础设施。** 所有部署共用一台内网 MinIO 的 S3 兼容 bucket；启动必须
  在应用/worker 之前严格解析配置并创建或验证 bucket 可达。endpoint 可为 HTTP 或 HTTPS；不得回退
  本地目录或按 edition 选择第二存储协议。供应商切换只改部署级配置，不得进入 AI Provider 配置。
