...
- 国际/港澳台模板生效前，手机号规范化接受任意语法合法的 E.164，腾讯云 adapter 却始终发送同一个国内模板和国内签名；UI 因“号码格式有效”暗示已支持境外目的地，真实境外调用只能被 Provider 拒绝。根因是从号码语法猜测业务 capability，没有目的地 registry，也没有把模板 scope 与 Sender ID policy 纳入唯一发送裁决。当前由 `sms-destinations.ts` 穷尽首批目的地，`tencent-sms.ts` 按 registry 唯一选择国内/国际模板、签名与 Sender ID；未启用目的地和缺少强制 Sender ID 都明确失败，不再回退。腾讯云套餐/按量计费状态、各运营商真实送达和尚未注册的专属 Sender ID 仍是外部盲区。
