...
- OpenRouter reasoning 请求曾只携带 effort；支持隐藏推理但需要显式 summary 请求的模型即使完成了推理，也不会产生可公开展示的 reasoning part。当前唯一 OpenRouter LLM adapter 在启用 reasoning 时同时发送精确 effort 与 `summary:auto`，SDK 返回的公开摘要继续进入统一 stream/result 协议；模型仍未返回摘要时不得由 Provider 层生成、日志反推或二次调用补造内容，Assistant 只能展示权威 Run 状态和“无可展示文本”。
