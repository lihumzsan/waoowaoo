...
- Provider-bound 私有媒体投影的唯一共享入口：`src/lib/media/outbound-owned-media.ts`；图片、音频和视频只在 `src/lib/media/outbound-{image,audio,video}.ts` 追加各自 MIME 与大小 policy，再输出 HTTPS URL。产品媒体协议的最终 HTTPS guard 是 `src/lib/ai-exec/media-references.ts`；adapter 内部 inline 转换不改变该协议。
...
- 2026-07-19 统一 AI SDK 协议时，OpenRouter request shaper 从解析每一步真实 wire body 改为读取模型创建时的 `input.messages` 快照，并用该快照覆盖 SDK 请求。普通同步调用会传快照，既有 contract 因而继续通过；Primary Agent 与 Creative Worker 由 Agents loop 在模型创建后动态增长 messages，创建时没有该快照，导致 Claude 顶层 `cache_control` 整体缺失、每一步按全量输入计费。旧防线只观察显式 cache block 的同步请求，没有覆盖 `executionMode=agent` 的真实 SDK wire 组合。当前唯一 OpenRouter fetch 出口以本次 wire body 为消息事实：Claude 在没有显式断点时自动加入 5 分钟顶层缓存，显式 Claude/Gemini block 只能在与 wire role/text 完全一致后附加，禁止快照覆盖增长历史；provider contract 用同一模型实例连续发送增长对话并断言每一步保留真实 messages、session identity 与自动缓存。真实付费 Provider 的路由与命中率仍需发布前监控 `cached_tokens/cache_write_tokens`，不由 contract 伪造命中。
