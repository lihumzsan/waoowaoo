...
- **PG-01B — OpenRouter 模型一次声明。** OpenRouter 普通 LLM 共用同一 adapter 和 `openrouter-chat` 协议，但每个具体 model identity 的名称、价格、公开 reasoning 模式、推理强度集合与平台可见性必须在 provider-owned definition registry 声明一次，再自动派生 capability、pricing、API config 与 platform catalog。OpenRouter Image 的 GPT Image 2、Nano Banana Pro、Nano Banana 2 与 Nano Banana 2 Lite 同样由一个 provider definition 文件派生价格、能力与模型目录，并共用官方 Image API adapter；model-specific option schema 只声明真实差异。未知 model id 必须显式失败；不得把任意字符串直通 provider，也不得为同一模型手工维护平行目录。
...
- **PG-05 — 精确模型、零自动降级。** 不支持的模型、能力、输出、账户额度或 provider 故障必须在用户选择的精确 `provider::modelId` 原地失败。生产 capability catalog 当前不得声明 `providerRoute`，因此 GPT Image 2、Seedance 及其他同名能力都不能在 Provider 之间自动前进；Agent、业务层、adapter 和错误处理也不得自行重试成另一模型。typed pre-accept rejection 只证明本次未受理，不构成切换授权。
- **PG-05A — 通用 route engine 不等于生产路由。** 既有 route-set 与 durable invocation 代码保留为未启用基础设施，但没有 production catalog member 时只能投影一条精确选中路线。未来启用任何 route 必须作为独立架构变更，重新证明 capability、canonical options、冻结报价、Task/Resource identity、pre-accept 与 replay 全部等价；不得通过新增一条 registry 声明偷偷恢复 fallback。
...
- **PG-06C — Provider request identity 排除临时传输凭据。** durable request hash 必须包含媒体对象的 HTTPS origin/path、顺序和全部非媒体 canonical option，但必须从签名图片、音频与视频 URL 中移除 query/hash；S3 签名、过期时间和临时 token 只授权本次传输，不是业务输入。adapter wire request 仍使用本次新签发的完整 URL，identity projector 不得修改它。相同对象在下一 Task attempt 取得新签名必须命中同一 checkpoint；对象路径、引用顺序或真实 option 改变仍必须 hash 分歧并失败关闭。
...
- **PG-12 — Provider 与 Cloud 模型所有权服从部署模式。** `platform-key` 下 Provider identity、base URL、密钥和各产品角色的默认 modelKey 只由服务器环境配置与平台 registry 拥有；用户 API 配置的读取、写入和连接诊断必须在任何数据库或外部请求前拒绝，用户模型偏好写入同样失败关闭，相关配置 Operation 只允许 API channel，不能进入主 Agent 工具集。Cloud 运行时、项目初始化、用户偏好 View 和模型目录投影必须读取同一组 `getPlatformDefaultModels()` 结果，不得解释 `UserPreference` 中的旧模型字段或公开其他平台模型作为选择项。`user-key` 下个人设置继续是自定义 Provider 与默认模型的唯一 writer。Provider transport 只负责按已选配置发请求与可选代理转发，不得再按 DNS 结果、私网/保留 IP、metadata、HTTPS、重定向或本地 allowlist 建立第二套可用性裁决；本地部署者对自定义网络目标负责。图片、音频、视频、multipart 与 base64 仍必须经共享字节上限读取，禁止无界 `arrayBuffer/formData/Buffer.from(base64)`。
...
- **PG-14 — Provider 先按生命周期分类，再选择传输实现。** Runtime 只有 `sync final result` 与 `async durable job` 两种执行语义，图片、视频或 provider 名称不得代替该分类。同步接口优先由能保留参数 policy、proxy、安全上限、错误语义且可关闭提交重试的高层 SDK 执行；异步接口必须让项目分别调用 `submit/status/result`、在 submit 后立即持久化 canonical external id，并由 Temporal TaskWorkflow 调度同一 Task Activity 从 provider invocation checkpoint 恢复。任何 SDK 若隐藏进程内 polling/download、自动 fallback，或无法关闭不确定 POST 的重试，均不得接管该边界；保留项目低层 transport 不构成第二生命周期。OpenRouter Image 使用官方 SDK 的单次 `images.generate`：GPT Image 2 的 partial stream 只维持同一同步传输，Nano Banana 系列使用同一 endpoint 的 buffered result，二者都只有最终 completed image 才进入持久化。OpenRouter Video 使用官方 SDK 的独立 `generate/getGeneration`；Toonflow 使用显式 `/video/generateVideo` 与 `/video/getVideoStatus`，external id 立即持久化；FAL Queue 因官方客户端当前固定重试 submit，继续使用零自动重提的项目 transport。
...
- 同步 OpenRouter Image 的唯一外部协议入口：`src/lib/ai-providers/openrouter/image.ts` 的官方 SDK `images.generate`；OpenRouter Video 的唯一 submit/status 入口：`src/lib/ai-providers/openrouter/video.ts` 的官方低层 SDK；Toonflow Video 的唯一 submit/status 入口：`src/lib/ai-providers/toonflow/video.ts`。这些入口只由各自 adapter 暴露，不允许业务调用方直连 SDK/HTTP。
...
- 模型目录、价格、能力和运行时选择：`src/lib/ai-registry/`；平台完整模型目录、用户官方选择 policy 与有效偏好解析：`src/lib/platform-models/`、`src/lib/ai-registry/platform-models.ts`、`src/lib/config-service.ts`。
- Provider route set 的未启用通用解析：`src/lib/ai-registry/provider-route-set.ts`。production capability catalog 当前无 route member，所有媒体调用只产生一条精确选中路线；`src/lib/task/provider-invocation.ts` 仍负责同一提交 fence 与结果重放。
...
- `npm run test:critical:provider` 与目标 provider contract 保留真实 adapter/wire 边界：FAL、OpenRouter 与 Toonflow 的提交/轮询/终态、零隐式 POST retry、模型能力、连接和 hosted web search 的结构化证据协议。
...
- Seedance 参考声音首次接入时，图片已通过 owner-aware storage normalizer 输出 Base64，声音却单独调用浏览器签名 route，并把 MinIO 返回的相对 `/api/storage/sign?...` 原样交给 Provider；真实 OpenRouter 请求因 `audio_url.url Invalid URL format` 在受理前返回 400，而无声音的同批任务正常得到 202。第一轮修复把两者都改成 Data URL，虽然删除了 session 旁路，却仍假设图片与音频拥有相同 wire 能力；OpenRouter 的声音字段要求 HTTPS，且并行大请求继续承担完整媒体上传时延。当前部署只接受外部 HTTPS S3-compatible endpoint，私有图片/声音/视频共用 owner→HeadObject→MIME/size→presign 投影，Gateway 在 provider fence 前强制 HTTPS；FAL/Ark/OpenRouter Video 不再上传 Base64。旧 local/MinIO/`/api/files` 传输和产品 Base64 协议均已删除。真实付费 Provider 对目标对象存储域名的可达性仍是发布复验边界。
...
- OpenRouter GPT Image 2 在真实调用中返回账户 billing hard limit 后，曾用 production `providerRoute` 自动前进 FAL；随后 Seedance 接入又准备复制同一模式。虽然 durable fence 能约束不确定 POST，它仍违背“用户选定模型就是实际模型”的产品语义，并让 Provider 成为失败时的隐藏状态解释。当前删除全部 production `providerRoute` 声明：typed pre-accept rejection 仍保留准确失败分类，但只结束所选 Provider 的本次调用，不授权换路；route engine 只剩未启用基础设施，registry conformance 反证当前不存在生产自动路由。
- Cloud 首版用环境默认覆盖 `platform-key` 的模型字段，后来又在个人中心加入官方 allowlist 和 `UserPreference` 写入，令环境默认与用户持久偏好共同解释同一运行时 modelKey。产品重新采用平台固定模型后，当前删除 selector、allowlist 和用户偏好解释分支：`getPlatformDefaultModels()` 是 Cloud 唯一模型事实，`update_user_preference` 拒绝全部模型字段，旧持久值保留但不参与运行时或 View。Provider 配置继续完全关闭；Self-hosted 的用户模型 writer 不变。
