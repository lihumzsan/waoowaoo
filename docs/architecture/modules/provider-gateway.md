...
- `tests/integration/provider/fal-provider.contract.test.ts`、`fal-queue-result-errors.contract.test.ts`、`fal-video-provider.contract.test.ts` 和 `elevenlabs-sound-effect-provider.contract.test.ts` 验证真实 provider 协议；FAL queue 的 malformed、FAILED、422、500 与 unknown 状态必须显式分类，不得进入 fallback。
