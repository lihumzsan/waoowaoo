...
| Task 冻结的方向 revision 与领域投影 | `delegate_creative_work` server compiler + output registry | 对应 Creative Worker、Resource lineage |
...
- 旧视觉 Bible 最初只拥有视觉字段，资产清单又通过显式 source material 与当前 Binding 二次核对，视频、音乐、剧本和章节路径则依赖 Primary 是否记得传递，形成多个注入者和不一致门槛。仅让所有 Subagent 主动读取 Project Context 仍会扩大权限、复制裁剪判断并让运行中 Task 受换绑影响。当前唯一 writer 仍是 `adopt_creative_direction`，唯一 Task 输入编译者是 `delegate_creative_work` 服务端：它一次读取 adopted Binding、按生产 registry 裁剪六领域并冻结进 `creative_work_v6` payload；Primary 手动传方向、Chapter conditional style、Asset 当前 Binding 核对和“方向必需”门槛全部删除。Conformance 从生产 registry 穷尽反证漏接，新旧协议不双读。
