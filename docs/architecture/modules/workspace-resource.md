...
- **WR-04 — 文件内容与语义按版本边界冻结。** 文本、结构化文档和媒体文件都由 `WorkspaceResourceVersion(resourceId, version)` 指向不可变 MediaObject。用户编辑只追加版本；生成结果由 Task terminal materializer 追加版本。`resourceId + contentVersion` 是执行输入身份，当前路径只用于显示和审计。包含注册 `outputKind` 的结构化文件必须在 checkpoint 通过同一 Creative Output schema 并获得固定 `project.*` schemaId；同一 Resource 不能改成另一 outputKind。文本/结构化版本在写入事务内同步物化有界 `contentPreview`，媒体版本为 null；preview 是派生展示列，不是第二内容权威。
...
- **WR-06 — Runtime checkpoint 是唯一文件写回入口。** Runtime materialize 时由 Catalog 和对象存储生成完整 Bundle；capture 时比较同一 baseline 并在一个事务内提交完整资源树。文件 identity 由文件内受保护的 Resource 标识维持；文件夹改名由本次 Runtime 的 inode identity 识别。新 JSON 没有 `outputKind` 时是 `generic.text`；存在该字段时必须由 Creative Output Registry 完整 strict parse，未知 kind、额外字段、缺字段或专业约束错误使整个 checkpoint 失败并返回精确 issue。并发路径、存在性、类型、schemaId 和 Agent 可写文本版本变化使整次 checkpoint 失败，不允许部分写回。媒体指针内容不可写且只编码稳定 Resource identity。
...
- **WR-10 — 批量生产是版本化专业 JSON，不是 N 个 Agent 调用。** `submit_production_manifest` 只接受项目相对 `manifestPath`，读取 ready 且 schemaId 匹配的 `asset_manifest | video_prompt_set | music_direction`，用同一 Output Registry schema 复验并冻结 `resourceId + contentVersion + workspacePath + sha256 + manifestId`；一次冻结最多 Registry 允许的显式条目、一个总报价和一个 Approval Grant，再由 Temporal 扇出 Task。空的 `no_assets/no_music` 决策不能提交。成员有稳定 `itemId/resourceId`；部分失败只由 `rerun_failed_production_items` 重跑失败成员，成功成员不重提。
...
- **WR-16 — 创作内容不在服务端编译。** 专业 JSON 必须包含完整最终 Prompt、创作身份与显式参数；资产 item 同时声明固定对应的 `assetKind/schemaId`、稳定身份字段和实际 `aspectRatio: 4:3`，视频声明画幅与时长，音乐声明时间窗、时长与 instrumental mode。Planner 在任何 Plan、报价、Resource 或 Task 副作用前按模态严格校验、选择正式模型、解析精确引用并冻结 Prompt/参数；禁止依据 `schemaId` 追加 Prompt、猜资产类型或覆盖比例。普通 `create_text` 只能创建 `generic.text`，不能给任意 JSON 伪造专业 schemaId；直接 `create_image/audio/video` 只保留 API/Canvas 通道和项目画幅，不进入主 Agent MCP。
...
- Production Manifest 上线时，Asset/Video/Music Skill 各自手写了一份字段示例，而 `productionManifestSchema` 又是另一份 strict 权威；真实 asset worker按 Skill 写出的身份字段被执行层拒绝。Zod issues 在模型投影中被剥掉，通用 path normalizer 还把 `manifestPath` 与 item 路径错误都写成 `outputPath`，导致模型把相对路径改为 Runtime 绝对路径并反复提交。当前三种媒体 outputKind 直接复用 Creative Output Registry 的同一 schema，manifestPath、outputPath 与 reference path 在 schema 层禁止绝对/Traversal；模型只看到有界 corrections，付费入口前的失败不创建 Resource、Task 或费用。
