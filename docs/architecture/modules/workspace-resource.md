...
- 提交前的专业 Prompt 校验只在日志保留原因，助手收到泛化失败后删减已确认素材和对白来求通过；上一版只覆盖输入 schema 的纠错，未覆盖专业 Plan 校验及持久工具输出边界 → 专业校验沿既有 corrections 通道保留输入身份、错误位置与纠正建议，纠正格式不授予改写来源内容或重放已受理任务的权限（WR-08/10/16）。
