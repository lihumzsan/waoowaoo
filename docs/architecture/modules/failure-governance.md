...
- **FG-04 — 投影不建立第二事实。** Task/Turn 仍是持久失败 owner；Provider adapter 与原生 Runtime
  必须传递同一份一手 FailureRecord。Temporal、checkpoint、API、Workspace 与 Assistant 只传递或投影
  该事实；最终 Turn 不能从 Runtime 包装文案反造第二份 Provider 失败。
