...
音乐、声音参考和混音结果都是普通 WorkspaceResource。创作判断由固定 `wao_music` 专业子 Agent使用唯一 `music-direction` Skill 完成，并把最终 Prompt 与显式参数写入 Production Manifest；主 Agent只按路径提交。最终混音只消费显式冻结的媒体与时间线。系统没有自动 BGM 阶段或声音工作流状态机。
...
- **AP-07 — 音乐输入是显式 Manifest 字段。** 音乐专业子 Agent写完整最终 `prompt`、`durationSeconds`、`vocalMode`、可选 `genre/mood/bpm` 与精确 context/video 引用；格式、模型和 provider option 由服务端 Registry/config 决定并在 Plan 前校验。Planner 不拼接 Prompt，只校验并逐字冻结；Task handler 只消费冻结结果。
