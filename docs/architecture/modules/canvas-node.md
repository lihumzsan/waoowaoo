...
- **CN-02C — 下游节点只消费 Workflow 能力。** 普通投影中，BGM 只在 `audio_plan`，最终时间线只在 `final_render`。历史资源或 active Task 可以稳定物化节点，但动作必须仍来自 `WorkflowView.operationPolicy.allowedOperationIds`。
...
- `tests/golden-journey/journeys/mainline-complete.spec.ts` 在真实主链中对齐 `ProjectVideoSegment` 数量、节点/播放器数量、BGM/最终时间线和刷新后 identity，并断言旧环境音节点始终为零。
