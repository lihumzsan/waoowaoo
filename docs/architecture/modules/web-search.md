...
- **WS-10 — Citation 只读结构化 annotation。** 来源 identity 只从 Agents SDK hosted output 的 `url_citation` annotation 读取并按 HTTP(S) URL 去重；不得从 Markdown 链接、报告正文、模型声称的引用或网页内容猜测来源。没有结构化 citation 必须 fail closed。
...
- OpenAI Search Agent、hosted tool、query/citation 投影与错误归一：`src/lib/ai-providers/openai/hosted-web-search.ts`。
...
- `tests/integration/provider/openai-hosted-web-search.contract.test.ts` 在付费外部 runner seam 注入真实 Agents SDK item shape，验证 hosted query、结构化 URL citation、去重、报告边界、缺 key、拒绝凭据和无证据 fail-closed。
...
- 真实 OpenAI 的中文论坛覆盖、登录墙、来源排序、延迟、成本和 Agents SDK annotation 稳定性属于发布复验；本地 contract 不伪造这些质量结论。
