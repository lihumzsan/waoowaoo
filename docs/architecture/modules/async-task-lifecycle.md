...
- **TL-14 — Durable command、checkpoint 与 resource row 只存有裁决力的事实。** Outbox command 由 `id + kind + payload identity` 裁决幂等与分派，provider checkpoint 由 `taskId + stepKey + invocation identity/hash/status` 裁决提交与重放；整集声音规划由 `ProjectEditAudioDesign.taskId + designSignature + timelineSignature` 裁决，MusicScore/AmbientSound 只以当前生成 `taskId + designSignature + timelineSignature` 裁决候选与 mix，不再保存 `planTaskId/planJson`；ProjectVideoSegment 由 `(editScriptId, segmentId) + inputSignature + generationTaskId` 裁决。规划与媒体生成是独立 Task 时不得用同一可覆盖字段冒充两个 owner。不得在 DB 行和 JSON payload 中重复持久化永远固定、没有 writer/reader 分支的 `version/contractVersion`。真实并发 fence 必须保留；不兼容形状变更禁止双轨解析。
...
| AudioDesign 规划与 MusicScore/AmbientSound 生成所有权 | AudioDesign worker 写唯一 plan；生成 worker 只写各自产物的 `taskId + designSignature` | Canvas 由 AudioDesign task 交接统一 stream；生成 projector 只围栏当前产物 |
...
- `tests/integration/task/task-attempt-claim.integration.test.ts`、`task-reconcile-queue.integration.test.ts`、`task-target-terminal-{ownership,projectors}.integration.test.ts` 与 `audio-design-owner-fence.integration.test.ts` 验证并发 attempt owner 的穷尽 claim、queue unavailable、真实 Redis terminal delivery 后保留 external id/target owner 的同 Task 恢复、late terminal、统一声音规划 owner 和唯一业务 projector。
...
7. `20260715160000_unify_audio_design` 删除声音资源的 `planTaskId` 与 Ambient Sound `planJson`，并新增唯一 `ProjectEditAudioDesign` 和生成资源 `designSignature`。应用前必须排空旧 `music_score_plan`、`ambient_sound_plan`、两类生成 Task 与 final render；迁移和新 Task registry 必须一次性切换。本仓只创建 migration，未授权也未执行任何共享数据迁移。
...
- BGM 与 Ambient Sound 拆成“免费文本规划 → 收费媒体生成”后，第一次修正给两个资源各加 `planTaskId`，虽然避免生成 `taskId` 覆盖规划 stream owner，仍留下两个 plan writer、两个状态解释和跨计划不一致。旧分层测试只分别验证 Task 终态，没有证明同一 AudioDesign 被两个生成链消费。当前规划收敛为一个 `ProjectEditAudioDesign` 和一个 `AUDIO_DESIGN_PLAN` owner；MusicScore/AmbientSound 删除 plan writer 与 `planTaskId/planJson`，只持有生成 Task 和设计签名。Golden 同时断言单规划 Task、两个付费 Task、签名一致、候选数量与连续 presentation。
...
- BGM 的 `music_score_plan` 曾同时在 music worker 内生成文本规划和付费音乐，后续虽拆为文本 Task 与媒体 Task，又与 Ambient Sound 各自形成一条规划状态机。现在唯一 AudioDesign 文本 Task 负责 `planning → planned`；两个媒体 Task 各自只负责 `generating → completed`，取消均回到可重试 `pending`，失败/取消 projector 不修改冻结设计。该 TaskDefinition 与 schema 切换不兼容旧活动声音 Task，发布前必须排空并在同一维护窗口应用 migration。
