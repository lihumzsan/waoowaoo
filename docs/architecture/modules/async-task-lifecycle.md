...
| Episode 聚合快照严格顺序                          | `ProjectEpisode.resourceRevision` / 数据库 trigger                                 | Query DTO seqlock、Terminal materializer、Query Cache       |
...
- Task 静态契约（queue、worker handler、billing、materializer、retry、execution protocol、success/failure/cancel handoff）：`src/lib/task/definition.ts`；新增 TaskType 必须通过穷尽 `satisfies` 与 conformance。
...
- `tests/contracts/task-definition-conformance.test.ts` 验证每个 TaskType 的 queue/handler/billing/materializer/retry/execution/success/failure/cancel 声明被必跑 guard suite 收集，并阻止 worker、Billing、materializer 恢复 TaskType 私有 switch。
...
- `tests/integration/task/workspace-resource-revision.integration.test.ts` 在真实 MySQL 上验证数据库拥有的 `ProjectEpisode.resourceRevision` 覆盖同一时间精度内的多次子表写入、物理删除和 Panel 间接关联；`scripts/guards/materialized-resource-version-guard.mjs` 穷尽检查正式 DTO 子表的 INSERT/UPDATE/DELETE trigger。
