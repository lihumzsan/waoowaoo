...
- **TL-06E — Creative Subagent 只使用 Task reference projection。** `creative_work` Task 是 Creative Subagent 的唯一持久容器：一个逻辑请求对应一个 Task，`Task.id` 是 Subagent identity，`Task.status` 是生命周期裁判，完整 strict Worker 输出只写 `Task.result`。TaskDefinition 必须把该类型的 lifecycle payload 与 continuation result 声明为 `reference`；created/progress/terminal Event 只携带有界 lifecycle projection，其中 reasoning 以 attempt-scoped 稳定 identity 原位更新，`read_skill` 只保存调用状态和版本/URI/字符数结果元数据；Wait continuation 只携带 `requestKey + outputKind + summary` 等引用摘要。主 Agent 需要完整输出时必须按 taskId 走正式 Task 查询。Creative Worker attempt 还必须受明确墙钟上限约束，超时中止模型 signal 并按 typed failure 进入既有 retry/terminal；heartbeat 不能让永久挂起的 provider 调用无限保持 processing。禁止把完整剧本、Bible、Chapter context、Skill 正文、系统提示、provider 签名或 Worker JSON 复制进 TaskEvent、Session、Wait、Outbox continuation 或 Assistant message；reference 缺失必须原地失败，不能回退为 full payload。
- **TL-06F — Creative 结果采用必须重新验证 Task 与领域来源。** Creative Task 完成不直接写业务事实；采用 Operation 必须在自己的唯一写事务中重新读取同 user/project/episode 的 completed Task，严格解析 payload/result，核对 output kind、调用方稳定 request identity 与精确来源 provenance，再把 `Task.id` 作为领域 generation owner 绑定并调用既有唯一领域 writer。`adopt_edit_bible_bundle` 还必须验证 SourceDocument id/version/checksum/完整 normalizedText，在同一事务内写 Bible 与唯一 splitter 的 Chapter；相同 Task 的重复采用返回同一正式事实，不能生成新版本。采用完成不自动提交任何下游 Task。
...
| Creative Subagent 身份、生命周期与完整输出         | `creative_work` Task 的 `id/status/result` / Task submitter、text worker、Terminal Service | Session Subagent View、正式 Task 查询；Activity/历史消息无解释权 |
| Task 生命周期与 continuation 的体积边界             | TaskDefinition `lifecyclePayloadProjection/continuationResultProjection` / `task/result-projection.ts` | TaskEvent、SSE、Wait、Assistant continuation                 |
...
- Creative Subagent Task 契约与执行：`src/lib/creative-worker/task-contract.ts`、`src/lib/workers/handlers/creative-work.ts`；Task type/handler/retry/projection 声明仍只在 `src/lib/task/definition.ts`。Worker 没有领域写权，完整结果留在 Task.result。
- Task created/terminal lifecycle 与 Assistant continuation 的唯一体积投影：`src/lib/task/result-projection.ts`；`transactional-create.ts`、Terminal Service 与 `project-agent/waits.ts` 只能调用该入口，不能按 TaskType 私自裁剪或回退 full result。
...
- Worker 曾在每次外部 pending 查询后，对同一 `polling_external` 进度同时写 Task/SSE 事实和 INFO 日志；进度通知又使 Outbox dispatcher 为每次成功投递输出一条 cycle INFO。并行长视频因而看起来像后台在反复刷新或重新生成。当前 Task progress 持久化、lifecycle publish、Outbox 投递与终态仍完整执行，但例行 progress 和成功 dispatch cycle 不再写 INFO；进程开始/完成、重试、恢复、异常与 dead-letter 日志仍保留。
