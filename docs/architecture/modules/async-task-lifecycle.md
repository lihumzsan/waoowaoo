...
- **TL-06E — Creative Subagent 只使用 Task reference projection。** `creative_work` Task 是 Creative Subagent 的唯一持久容器：一个逻辑请求对应一个 Task，`Task.id` 是 Subagent identity，`Task.status` 是生命周期裁判，完整 strict Worker 输出只写 `Task.result`。TaskDefinition 必须把该类型的 lifecycle payload 与 continuation result 声明为 `reference`；created/progress/terminal Event 只携带有界 lifecycle projection，Wait continuation 只携带 `requestKey + outputKind + summary` 等引用摘要。主 Agent 需要完整输出时必须按 taskId 走正式 Task 查询。Creative Worker attempt 还必须受明确墙钟上限约束，超时中止模型 signal 并按 typed failure 进入既有 retry/terminal；heartbeat 不能让永久挂起的 provider 调用无限保持 processing。禁止把完整剧本、Bible、Chapter context、Skill 正文或 Worker JSON 复制进 TaskEvent、Session、Wait、Outbox continuation 或 Assistant message；reference 缺失必须原地失败，不能回退为 full payload。
