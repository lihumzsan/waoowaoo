...
Codex app-server 是唯一 Agent Runtime，并独占模型采样、原生搜索、Thread/history 与当前用户的标准 Codex Home；Wao 保留产品
View、WorkspaceResource、Capability Service、计费、审批、Task 和执行平面。桌面原生模式直接复用当前 Windows 用户的 Codex 登录，
Wao 只为每个 placement 物化可销毁 workspace 与其中的 registry Skills，不写入或清理 Codex Home。Runtime 可被替换，因此 UI 和业务
...
- **CRR-02 — 适配器隔离协议。** UI、route 和业务 service 只能通过 AssistantRuntime；协议方法、
  ...
  并验证，不能只在会话层声明后假定已存在。
- **CRR-03 — 桌面原生模式不静默降级。** 当前桌面登录模式只允许显式 local driver；Codex 只能写临时
  Project workspace，标准用户 Home 由原生 Codex 自己维护，Wao 不复制、挂载、清理或替代它。Docker
  不是该模式的 fallback；未来多租户容器若重新启用，必须由独立部署契约声明双层隔离与资源边界。
- **CRR-04 — WorkspaceResource 才是项目持久事实。** Runtime 只物化空 scratch 与 workspace 内 registry 生成的主 Agent Skills；
  ...
  标准 Codex Home 只由原生 Codex 拥有 Thread/history 与用户配置，Wao 不写入 Resource、Skill 正文或运行时配置；scratch、inode 和
  Runtime 路径都不是产品权威。
...
- **CRR-06 — Runtime 状态与产品 View 分权。** 当前用户的标准 Codex Home 是模型 Thread/history 与 resume 的
