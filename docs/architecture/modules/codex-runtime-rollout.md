...
- **CRR-07B — Runtime bearer 不是模型 API key。** Bearer nonce 同时是当前 Runtime placement 的 Redis ownership token；Responses、standalone search 与 MCP 必须通过同一个 capability Turn resolver，先证明该 nonce 仍持有租约，再证明 Project 恰好一个活跃、未取消、Thread 未 clear 且 `runtimeTurnId = executionOwnerId` 的 Product Turn。执行中不得要求 `runtimeThreadId`：它只在 Codex 状态成功 checkpoint 后作为恢复身份绑定，新 Thread 的首个活跃 Turn 合法地没有该字段。停止/轮换/清空 placement 后旧 token 即使未过期且后来出现新 Turn 也不能重放。父 app-server 可访问内部网关，但 shell/patch 在 local 与 Docker 中均使用 `workspace-write + networkAccess=false`。
...
- 首版协议 smoke 只安装一个 synthetic Skill，七个正式 Skill 缺少原生 frontmatter 时仍能通过；后续又把全部生产 Skill 安装给主 Agent，无法证明角色选对。当前 smoke 证明主 Agent零 Wao Skill，并从生产 Worker Registry 穷尽物化固定 custom agent、精确 Skill 集与禁用 Wao MCP；真实组合验收继续覆盖 `workspace write → MCP → pending Resource → checkpoint`。
- Runtime smoke 曾以未被事件循环持有的顶层 Promise 启动，异步验证尚未执行完进程就以 0 退出，导致空输出被误当成功；MCP 用例还硬编码了现已删除的 `create_image` 入口。当前 smoke 显式保持进程到完成、为每阶段设置失败超时，并只调用生产 Registry 当前唯一的 Manifest 媒体入口。
- 固定专业 Worker 上线时，协议 smoke 只验证代理文件和 Skill 内容，没有验证普通用户创作请求能越过 Codex 默认 `explicitRequestOnly` 委派门槛；提交说明也把真实 child spawn 明确留作部署盲区。主 Agent随后因“必须委派”和“用户未显式要求 Subagent”同时成立而停止。当前 Codex home 物化全局 `AGENTS.md` 作为官方支持的自主委派授权，显式开启 `agents.enabled` 并提升 Runtime revision；不使用已废弃的 `multiAgentMode` 参数，也不把全部请求强制设为 Ultra。
- 固定 Worker 的首版 profile 还用无 transport 的 `[mcp_servers.wao] enabled=false` 表示禁用，Codex 会把它判为 `invalid transport` 并忽略整个 agent role；只匹配 TOML 字符串的 smoke 无法反证。当前 profile 提供禁用的 loopback 空 transport 并由真实 Codex 启动检查解析结果，子 Agent仍不获得 Wao MCP。
- 固定 custom agent 不能与全历史 fork 同时选择：全历史 fork 必然继承 parent agent type。自主委派指令因此固定使用 `fork_turns="none"`，由 parent 显式传递本次所需路径与范围，避免先失败再由模型猜测重试。
