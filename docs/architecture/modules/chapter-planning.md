...
- **CP-02A — Segment 声音时间线是可选的原生音频提示。** `synchronousSound` 继续属于单个 Shot，描述当前画面内的同期声；`generationSegments[].soundCues` 才描述一次视频模型生成内部的相对时间范围，可表达声音先于画面来源、跨 Shot 延续或画外对白先揭示人物。没有明确叙事价值时必须输出空数组，不得为了形式强加。`sourceShotRef` 只可指向同一 Segment 内拥有该对白/声音来源的 Shot，解析后变为 canonical `sourceShotId`；它只进入该 Segment 的视频 Prompt，不创建独立环境音/SFX 状态或跨独立 Segment 的预播放承诺。
