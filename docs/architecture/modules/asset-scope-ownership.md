...
- **ASO-10 — 顶层资产删除与图片生成各有一个入口。** 角色、场景和道具的顶层删除只允许 `delete_asset → removeAsset`，Project/Asset Hub/API 不得保留按 kind 分裂的 delete Operation；appearance/image 等 variant 删除仍按精确 parent/variant identity 独立处理。资产图片只经 `create_image.request.kind=asset|manifest_assets|retry`：`retry` 重新提交 exact failed Resource 的冻结输入；用户要求不同结果时，现有 `asset|manifest_assets` 分支为同一资产 identity 创建新的不可变图片 Resource，并由终态 CAS 决定当前图。系统没有独立 `regenerate_asset` Operation、候选槽或第二图片 planner。
...
| 资产图片生成与失败重试 | `create_image.request.kind=asset|manifest_assets|retry` → 同一 planner/commit | 精确 asset/variant；新结果创建新 Resource，failed retry 沿用原 Resource；不创建顶层资产 |
...
- 顶层资产删除曾同时存在 Project character/location、Asset Hub character/location 和 generic location/prop 五个 Operation，部分路径直接调用 Prisma delete，Agent 又有按图片组/单图分裂的旧重生成工具。删除随后收敛为 `delete_asset`，但 `regenerate_asset` 只留在架构文档与 Operation 标题文案，生产 registry 没有实现；继续保留会让模型和维护者误以为存在第二图片入口。当前删除该死协议，已有 `create_image` 同时拥有新图片生成与 exact failed Resource retry；variant 删除仍保留其精确 parent/variant 契约。
