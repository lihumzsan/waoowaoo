...
- **AP-13 — 真人视觉安全政策只有一个正文。** `HUMAN_VISUAL_SAFETY_POLICY` 是真人、真实人脸、公众人物、脸部相似物、数字人和照片写实人物限制的唯一文字权威。Primary System Prompt 只通过 catalog 声明的变量投影该正文；`delegate_creative_work` 只按 output registry 的 `requiresHumanVisualSafety` 向 `creative_direction`、`asset_manifest` 与 `video_prompt_set` 请求注入同一字符串，Skill、Operation 描述和媒体 Compiler 不得复制或改写。该政策只约束真人视觉呈现，不建立角色、作品、品牌、商业 IP、版权风格或歌曲模仿限制；Provider 自有审核拒绝继续走统一 typed failure。
...
- 人物非写实安全规则曾只出现在 Primary Prompt 末尾，但 Creative Worker 不读取 Primary System Prompt；随后又分别写进 Primary、Creative Direction Skill 与 `systemConstraints`，形成多份可漂移正文。一次范围过宽的删除又把真人限制和 IP 限制同时移除。当前只恢复真人视觉安全：唯一正文位于 `HUMAN_VISUAL_SAFETY_POLICY`，由 Prompt 变量和视觉 Worker registry 两处投影；Creative Skill、网页参考图 Operation 与媒体 Compiler 不再复制政策。角色、作品、品牌、商业 IP、版权风格和歌曲模仿限制保持删除。真实模型服从度仍是生成复验盲区。
