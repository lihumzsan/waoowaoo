...
- H3 Prompt 曾删除固定无字幕约束，随后参考生成又切到会稳定烧录字幕的 T8 图；上一版只恢复并校验
  Prompt 是否含有弱约束，没有对当前 Provider 图的原始 MP4 做回归 → 含对白批次仍普遍生成硬字幕 →
  video-direction Skill 作为唯一 writer 写入独立强约束行，提交边界拒绝缺失或显式字幕载体，参考生成回到
  经真实样片验证的单一 Ref2VA 图（APO-05/08）。
