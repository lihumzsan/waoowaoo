...
- **AP-10 — Creative Worker 输出协议穷尽且与 Task 分层。** Creative Worker 结构化输出只由 `creativeWorkOutputRegistry` 声明，当前只有 `screenplay_draft`、`edit_bible_bundle`、`continuity_analysis`、`style_bible`、`asset_prompt_set`、`video_prompt_set`、`music_direction` 与 `creative_review`，不存在 `story_analysis`。`style_bible` 区分 finalized 与 candidates；final 只能经 adopt Operation 成为结构化 Resource。`asset_prompt_set` 区分稳定身份、最终生图 Prompt 与可选风格来源。`video_prompt_set` 必须在同一 strict 结果中表达权威 aspectRatio、总时长、全局导演方向、各生成段时长、逐时间段导演执行和可直接交给视频模型的 `finalPrompt`。Primary 只传总时长，服务端注入允许时长；final parser 必须验证准确画幅、时长枚举、连续时间线与总和。导演知识与视频模型表达可以来自不同 Skill，但不能拆成两个隐藏必经模型协议。完整结果只保存于 Creative Task result，Task lifecycle 与 Assistant continuation 只携带 reference projection。
...
- **AP-12 — Worker 自己选择 Skill，Primary 只描述创作目标。** Worker 初始上下文必须包含生产 Skill registry 的完整紧凑 catalog，`creative-core` 固定预载，唯一知识工具是 `read_skill`。Primary 的委派输入、Operation schema、output kind 和服务端映射都不得携带 `requiredSkillIds`、推荐 Skill id 或固定专业角色；Worker 必须根据 catalog 的标题、摘要、标签和适用范围自主读取至少一个专业 Skill。主 Agent Prompt 只需说明“专业创作应委派”，不能复制 catalog、Skill 正文或选择算法。
