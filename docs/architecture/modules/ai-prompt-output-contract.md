...
- `scripts/guards/prompt-i18n-guard.mjs` 验证 catalog、locale 与变量；`prompt-semantic-regression.mjs` 拒绝固定 mainline、确认剧本、固定分段配方和专用方向 Choice 回流，并锁定 `>15s` Primary 规划纪律、用户时长分配、按需 Operation 调用、方向 lineage 与人物非写实约束。
