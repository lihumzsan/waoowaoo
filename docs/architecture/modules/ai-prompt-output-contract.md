...
- H3 Prompt 为贴近正向描述一度把固定无字幕约束改成“完全省略文字层元描述”，执行边界又没有校验
  这项用户约束 → 含对白的新批次普遍生成硬字幕 → video-direction Skill 作为唯一 writer 固定写入约束，
  提交边界只校验缺失并显式失败，不代写或改写 Prompt（APO-05/08）。
