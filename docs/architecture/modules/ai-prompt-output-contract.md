...
- H3 Prompt 曾删除固定无字幕约束；首次纠正把少量样片中的烧录字幕直接归因于 T8，并把核心 Ref 图回退
  到旧 Ref2VA，既没有隔离 Prompt 与图内模型变量，也偏离了已确认的 Ref 产品主路径 → Prompt 输出约束与
  Provider 工作流分别由各自权威入口治理；Ref 保持单一 T8 图，字幕复发只能在 T8 内做单变量验证，不得
  通过替换工作流、fallback 或其他输入模式规避（APO-05/08）。
- H3 无字幕规则曾以关键词出现代替文字用途判断，误拒合法否定句却放过同步对白文字，上层提示又继续许可字幕载体 → 逐字内容与协议语义必须分开，导演 Skill 统一文字槽规则，执行层复用同一字面量边界校验明确用途；固定约束不能由生成内容自行授权，也不能保证模型成片绝无字幕（APO-05/08）。
