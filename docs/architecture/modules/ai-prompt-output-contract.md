...
- **AP-13 — 项目事实、候选浏览与精确内容分层。** 主 Agent 每轮初始项目快照和 `get_project_context` 只提供确认剧本、正式 Style Bible、角色音色等保留 Binding 的紧凑工作集；`list_resources` 用于浏览候选/历史；完整内容只能由 `get_resource(resourceId, revisionId)` 取得。Prompt 必须要求生成后通过 Resource-native confirm/adopt 建立正式事实，不能让模型凭对话记忆、最近 Resource 或旧专业实体猜当前版本。媒体工具把语义 lineage 写入 `contextReferences`；`create_image.imageReferences` 只接收真实图片，`create_video.mediaReferences` 只接收真实图片或声音 Revision。
