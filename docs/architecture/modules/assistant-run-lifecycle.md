...
- Active Operation 专用展示选择：`workspace-assistant-panel-state.ts` 的穷尽 presentation registry；视觉风格内容由 `style-preview-set-view.ts` 构造后交给独立只读 `EditStylePreviewGenerationDataCard.tsx`，renderer 无权读取 Query、Task 或消息快照。
