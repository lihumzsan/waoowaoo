...
- 收费 BGM 与环境音在审批恢复后虽然执行并创建了两个 Task，SDK Tool 卡的名称却被通用 `call` 覆盖，因而显示两条“已提交 · 项目操作”。名称 registry 并不缺失；真正丢失的是恢复流中的 Tool identity。`c47993138`/`aa23c77c9` 曾在缺少 input chunk 时从 callId 文本猜工具名，合成测试又使用包含 operationId 的 callId，因此没有覆盖真实 opaque identity；后续单独增加“已提交”View 则形成第二套 UI 消息协议。当前防线恢复唯一 SDK Tool 卡语义：首次调用由 adapter 登记精确关系，审批恢复从已持久化 member identity 重建，stream 对 generic/missing input 只接受该关系且冲突原地失败。主 Golden 的并行组改用不含 operationId 的 opaque callId，并验证审批前没有“已提交”、批准后恰好出现两条具体操作名、没有“项目操作”且同一 Wait 显示两个进行中 Task。拒绝、断线及外部真实模型的跨浏览器视觉时序仍是未单独执行的盲区。
