...
- **AR-02A — Choice 命令所有权显式。** 用户提交结构化 Choice 后，服务端必须在同一消费事务中锁定 current Run fence 与 pending interruption。registry 已声明且输入可由规范化 Decision 完整构造的确定性、非收费、事务型确认，是用户已经发起的命令：消费事务直接通过统一 Operation invocation 写入，不得再交给 AI 二次转发。其他创作/异步/收费 Decision 只恢复 AI 或正式 handoff，不得被服务端从 recommendation 猜测执行。事务提交后的 AI 重读正式资源与主链建议，自主决定下一次工具调用或完成；推荐动作不是必须耗尽的义务。
...
- **AR-02D — Choice 不获得第二写权。** 所有 Choice 卡片统一使用 `submit_tool_output`；renderer、Panel 与 route 永远不直接修改领域数据。确定性确认只能由 `EDIT_FIRST_CHOICE_REGISTRY` 映射到既有注册式 Operation，并复用其 schema、channel、transaction 与 write authority；视觉风格选择仍只由 `confirm_edit_style_preview` 写入。确认提交后必须重新读取正式领域/Resource 事实，禁止把旧快照与 Decision overlay 当作当前状态。
...
- **AR-02F — Choice 定义穷尽注册。** 每一种 Edit-first Choice 的 `choiceType`、tool identity、受审资源种类、Offer 构造能力、Decision parser/serializer、触发策略与原子确认命令 policy 必须由同一个穷尽 registry 定义。持久 Choice 卡片同时显式声明 `replyMode`、`submit.decision` 与每组 `presentation`；通用 renderer 只能消费这些策略，不得按 group key、候选内容或历史消息私自解释交互、决定或布局。`asset_review` 审核本集唯一共享资产集，因此 Offer 不声明章节 option group；章节 requirements 只进入受审资源 fingerprint，批准语义始终是确认整组资产。已有专用只读资源卡需要承载同一 Choice 时，Panel 只可使用 Session 中持久 Offer 的 `choiceType` 选择该 renderer，并必须复用通用 Choice output 与 control 提交入口；不得新增协议字段、领域 mutation 或第二张同步卡片。Offer 校验、命令提交和结果续跑只按 registry 分派；Choice 由 Agent/用户显式触发，Workflow recommendation 不得自动伪造 Offer。新增 Choice 只能增加一种明确实现并注册，缺失任一能力必须编译或 contract 失败。
...
- **AR-03C — 一个 Operation 的 Task batch 与唯一 Wait 原子交接。** Runtime 明确设置 `parallelToolCalls=false`，每个模型 step 最多执行一个 Operation；并行工作由该 Operation 的一个 Task batch 表达，不再由 Workflow 声明跨 Operation group。Task-producing Operation 必须复用 `prepareTaskSubmissionInput + persistSubmittedTaskBatchInTransaction`；全部 Task、billing freeze、Created Event、lifecycle/enqueue Outbox 与唯一 Wait membership 必须同事务提交，单 Task 也是 batch size 1。不可变计划声明的既有 active Task dependency 必须在同一批准提交事务加入该 Wait，但不得重新创建、入队或计费。Wait 行是本次 Operation batch 终态聚合的唯一事实：每个 Terminal transaction 持有 Wait row lock后只合并本次 `taskId + lifecycleType`，最后一个终态准备唯一 continuation handoff；不得每 Task 建 Wait、首个成员完成就恢复、提交后猜 membership 或用 Task 快照重建聚合状态。
...
- **AR-03A — 失败不自动授权改写，也不改写 Run 结论。** Task 失败事实只属于 Task/Wait/Activity；终态 continuation 必须把成功与失败的真实 Resource/Task refs交给 Assistant。模型可以据此形成一个新的显式重试决定，但必须重新调用正常 Operation；收费重试仍重新生成精确报价并等待 Approval，不能继承失败 Task 的授权。系统不得在没有新 Tool 决定时自动重写、重新提交或写领域事实。若解释/规划回合本身没有协议错误，continuation 可结算为 `completed`，禁止把被解释的 Task 失败提升为 `run.failed`。
...
- **AR-05A — Operation 副作用服从 Run execution fence。** Assistant Tool 调用必须把同一个 abort signal 与 `runId + runVersion + eventSeq` 交给 `invokeProjectAgentOperation`；continuation 还必须携带 `waitId + commandId + claimOwner`。统一入口在执行前拒绝已失效 Run；普通 Task 创建事务、批准计划事务与同步领域写入事务必须在 commit 前锁定 Run 行，并在 continuation 中同时锁定 Wait claim 行后再次校验 fence。每个 execution segment 只允许当前单一 Operation 造成的单调水位前进；必须验证 execution-started identity、Run 仍为 `running` 与水位不回退，不能把许可扩展到其他 segment 或终态 Run。心跳、Redis lock 或 continuation claim 失效后，即使 Operation 已经开始，未提交的领域写入也必须整体回滚；只在 execute 返回后检查状态不构成防线。

- **AR-05C — 收费 Operation 逐次冻结精确决定。** 一个模型 step 只有一个 Operation，因此每次收费 Tool call 只冻结该 Operation 的 exact input、不可变 plan、quote、tool/operation identity 与 serialized RunState。用户批准后只签发该计划的 Grant并恢复同一 Tool call；拒绝则不创建 Task。不得引入 Run 预算、自动批准、跳过确认或由后续模型扩大已批准计划。需要生成多个候选或并行媒体时，由同一个 Operation plan 声明一个 Task batch并在一张报价卡中准确计价。
...
- 主链 recommendation 纯投影：`src/lib/project-workflow/edit-first-view.ts`；数据库事实装配：`src/lib/project-workflow/edit-first.ts`。模型输入只接收 `mainlineStep + mainlineStatus + mainlineStatusReason + mainlineRecommendedOperation` 作为建议；不得恢复 `allowedOperationIds`、`operationGroup`、`isEnabled` 或固定 next-action 协议。
...
- Choice 身份、能力、确认命令与触发策略的穷尽入口：`src/lib/project-agent/edit-first-choice-tools.ts` 的 `EDIT_FIRST_CHOICE_REGISTRY`。
...
| 用户决定 | `interruption.resolved.response` 中的规范化 ChoiceDecision / `consumeProjectAgentChoiceInterruption` | registry 确认命令、正式 Resource/领域 View 与下一回合 runtime |
...
| 选定视觉风格的领域写入 | `confirm_edit_style_preview` Operation / `confirmProjectEditStylePreview` 服务 | 刷新后的 Resource、Edit Bible 与 UI 投影 |
...
- 付费 Operation Group 曾把多个 approval item 与同一 RunState 持久化，却只保存首成员 plan 并签发一个 Grant；collecting Wait 又引入跨 Operation 的 seal 时序。当前 runtime 每个模型 step 只允许一个 Operation，一次收费调用冻结一份 exact plan/quote/Grant；同一动作的并行候选进入该 Operation 的一个 Task batch 和唯一 Wait，不再需要 Workflow group 或合并不同动作的审批协议。

- Operation Group 最初只在 approval resume 后建立 collecting Wait，因此同一模型回合的并行 Tool 可能只等待部分 Task。当前 Agents SDK loop 关闭 parallel tool calls；每个 Operation 自己的 Task batch 在同一提交事务绑定唯一 Wait，Task terminal 聚合只处理这个明确 batch。
...
- 镜头执行计划完成后曾把确定性 Storyboard/Panel 投影暴露成新 `nextAction`，迫使 continuation 调用一个没有 provider invocation 的第二 Task。`994b738981` 只把模型实现改为纯函数，没有删除入口与解释权；后续全能参考又以旁路追加。当前 Storyboard/Panel/分镜图阶段已整体删除；专业主链可推荐 `generate_video_segments`，但 Assistant 也可用显式 Resource references 调用通用 `create_video`，两者都不由 Workflow 强制。
- 最终渲染失败分支早于章节成片和声音阶段建立；后续新增前置阶段时只追加在旧分支之后。真实恢复项目因此会在上游缺失时推荐必然失败的 `render_final_video`。当前 Workflow recommendation 只按最早缺失的 Segment、章节成片、BgmDesign 和 MusicScore给出主链建议，不再开放或关闭工具；`render_final_video` 始终可调用，并由自身 planner 对本次显式 episode、章节渲染目标和合成输入原地校验。
- Workflow 曾把约 30 个“位置 × 状态”组合编码成线性 stage，同时另外存 blocking、nextAction、allowedOperationIds 与 operationGroup；Canvas 又维护 stage rank 和 operation-id 可见性表。任何新 Operation 都要修改多个解释器，`render_chapters` 漏接一次便隐藏了已有产物。当前防线保留 `step/status/recommendedAction` 作为只读主链建议，删除 Operation allowlist/group、Canvas visibility resolver 和固定 continuation；toolset conformance 从生产 registry 穷尽证明所有授权工具可见，两个 Golden 分别证明专业主链与空项目自由创作。
...
6. Choice Decision 是否由 registry 明确区分“用户已发起的原子确认命令”和“仍需 AI/Task/Approval handoff 的创作请求”？确认提交后是否重读正式 Resource/领域事实，而没有服务器猜测或强制耗尽 recommendation？
