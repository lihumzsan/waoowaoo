...
- 2026-07-14 的视频链路重构把项目助手系统 Prompt 整体缩短，同时删除了“推荐操作同轮调用、`needs_user_choice` 创建持久 Choice、请求用户选择前不得以纯文字结束”的回合闭环。真实视觉风格任务因此先在图片审批前以错误完成文案停住；用户手动批准并完成三张图片后，continuation 又只说“请选择”而没有调用 `request_edit_style_choice`，数据库没有 Interruption，任务结束后只读预览卡按既有显示条件消失。旧 Golden provider 直接按 Workflow 位置硬编码正确工具，无法反证真实模型遗漏；旧 Prompt guard 也只检查结构化输出字段。当前防线恢复与规范 WorkflowView 一致的中英文闭环语义，并由 localized semantic guard 直接拒绝再次删除；Choice Offer 仍只由 registry/Interruption 唯一入口写入，服务端和 UI 不从文案、历史消息或 Preview 记录补写。真实外部模型是否始终服从 Prompt 仍是未验证盲区。
