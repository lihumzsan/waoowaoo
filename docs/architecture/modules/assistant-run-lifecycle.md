...
- **ARL-03 — 产品 View 独立持久。** 用户消息、assistant item、Plan、Goal、request_user_input、Web Search、命令、Diff、MCP、Skill、Subagent、审批与 compaction 由事件 projector 写入 MySQL View；刷新不读取 Codex 本地文件格式。一个 Turn 在每次已接受 steer 的用户消息处冻结当前 assistant segment，随后以同一 `turnId + attempt` 的递增 segment identity 继续投影，因此聊天顺序始终为“已发生的 Assistant 内容 → 用户追加 → 后续 Assistant 内容”；不得把用户追加挪到整段回复之前或之后。原生 Subagent 的 child Thread 生命周期与有界活动明细都由 child 原生事件投影，不占用或结算 Project 的 parent Turn slot。
...
- **ARL-09 — 产品只有 Agent 模式。** Wao 不向用户暴露 Agent/Plan collaboration mode 切换，也不把模式写入 Turn context。Codex 原生 Plan 仍可作为 Agent 执行过程中的结构化 View 出现，但不是另一种会话协议或第二个准入入口。
...
用户消息由一个发送入口决定 start 或 steer；先按 project-scoped 消息 identity 查询持久命令，再做附件准备和 active Turn 选路。新 Turn 的用户消息与 `turn/accepted` 命令原子持久化；steer 先写同表 `steer/pending`，projector 冻结当前 assistant segment，runtime 接受后在同一事务中确认并把用户消息写在该 segment 之后，后续事件进入下一个 segment。同 identity 的 accepted 请求只回放 receipt，未确认请求 fail-closed 且不重发；即使 clear 后重建 Thread，旧命令也不能进入新一代 Turn。start/resume 成功后开始 Turn，事件逐个投影，结束时 capture Workspace 与 Codex state。断线只影响 SSE 传输，刷新从 MySQL View 恢复。Manager 重启通过 ownership 与持久 binding reconcile；晚到旧 Runtime 事件因 Turn/runtime identity 不匹配而拒绝覆盖。
...
- 运行中模式按钮首版仍可切换，而服务端把带新模式的发送转换为 steer 后静默丢掉模式。产品并不需要两套对话协议；当前只保留 Agent 模式，Codex Plan 作为同一 Agent Turn 内的结构化执行 View，模式选择、锁定和冲突分支已删除。
...
- Steer 首版在 runtime 接受后把用户消息追加到 Thread 尾部；第二版把它固定插到单个 assistant 汇总之前，又把已经发生的思考与回复挪到用户追加之后。当前 projector 在 steer 处冻结已有 segment，用户消息写在边界之后，后续输出进入递增 segment；消息持久化仍以 runtime 已接受 handoff 为前提，失败不会伪造已送达。
- 原生 Subagent 首版只投影 child path 和终态，UI 因而只有“0 进行中 / 1 完成”的空壳，原生 spawn/wait 工具还重复占据多行。随后 projector 按 child thread/item identity 持久化有界 message、reasoning 与 tool 活动，但 UI 把它塞进工具行里的可折叠卡，协作细节被压在与低层调用同级的位置。当前面板顶部标签条是 child 生命周期的唯一渲染者：只列出仍 active 的 child，终态即关闭标签并落回主线程；全部原生 subagent 工具行（含 `subagent_activity`）在存在 lifecycle 投影时一律不再单独渲染，避免同一事实有第二个可分歧的表面。child Turn 事实仍不成为第二个 Product Turn。
