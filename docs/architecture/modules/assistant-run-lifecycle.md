...
- 最终渲染失败分支早于章节成片和声音阶段建立；后续新增前置阶段时只追加在旧分支之后。真实恢复项目因此会在上游缺失时重新开放必然失败的 `render_final_video`。当前唯一 Workflow 先裁决最早缺失的 Segment、章节成片、BgmDesign 和 MusicScore，只有全部前置满足后才开放最终合成。
