...
- **AR-01F — Creative Worker 不形成第二 Run。** 主 Agent 只能通过注册式 `delegate_creative_work` 委派专业创作推理；Worker 是同一 Tool call 内的一次性、无状态模型循环，只能发现和读取 Creative Skill。它没有 ProjectAgent Run/Thread、Operation、Task、Wait、Approval、Choice、Resource、Canvas 或数据库写权，结果也不裁决项目状态。真实执行仍由主 Agent 的现有 Operation registry 单入口完成；Worker 超时、失败或取消只结算本次 Tool call，不能启动 continuation 或后台恢复。
...
- `tests/golden-journey/journeys/mainline-complete.spec.ts` 是专业主链跨浏览器、UI、Agent SDK、Operation、MySQL、Redis、worker、Outbox、SSE 与刷新恢复的最高组合证据；`freeform-resources.spec.ts` 的并行 OperationBatch 场景从一个模型步骤发出三次同名 `create_image`，验证一张报价卡、三份精确 Grant、一个后台 Wait、三个 Resource/Canvas 节点与一次续跑。
