...
- `tests/golden-journey/journeys/stage-probes.spec.ts` 验证 Workflow Lab 的 11 个稳定 checkpoint 经过生产 list/fork、真实 Session/UI 动作和持久 oracle 后能到达下一边界；Approval 还必须可重复 fork 后消费。`tests/unit/workflow-lab/checkpoints.test.ts` 只反证 interruption identity、runState admission、reserved identity 与 stage normalization 的纯逻辑边界。
