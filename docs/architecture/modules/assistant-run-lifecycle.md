...
- 原生 Subagent 首版只投影 child path 和终态，UI 因而只有“0 进行中 / 1 完成”的空壳，原生 spawn/wait 工具还重复占据多行。当前 projector 按 child thread/item identity 持久化有界 message、reasoning 与 tool 活动，UI 用一张可折叠 Subagent 卡展示并隐藏重复的低层协作调用；child Turn 事实仍不成为第二个 Product Turn。
