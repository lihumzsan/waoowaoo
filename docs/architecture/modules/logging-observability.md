...
- **LG-07 — 语义 action 由守卫脚本冻结。** 关键链路的 action 命名（`api.request.*`、`worker.*`、`sse.*`、`task.submit.persisted`/`task.submit.enqueued`）由 `scripts/check-log-semantic.ts` 做存在性检查并接入 `verify:push`；重命名 action 必须同步该脚本。
...
- 服务启动挂载：`src/instrumentation.ts`。
- 守卫：`eslint.config.mjs`（no-console）、`scripts/check-log-semantic.ts`（经 `scripts/verify-push.sh`）。
...
| 执行上下文（requestId/taskId/attempt/runId 等） | `context.ts` AsyncLocalStorage | `core.ts` 合并进事件；（架构债）billing 幂等与 provider fence |
...
- `npm run check:log-semantic`（含于 `npm run verify:push`）：关键 action 命名存在性。
...
- 自研 `check-no-console.ts` 未接 CI、allowlist 过时，且 `rg` 缺失时静默通过（守卫本身静默降级）。当前由 ESLint `no-console` 承担（LG-06），语义脚本改为文件缺失即显式失败。
...
6. 重命名/删除关键 action 时是否同步 `scripts/check-log-semantic.ts`？
