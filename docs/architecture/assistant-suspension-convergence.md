...
| 协议 | 持久事实 | 恢复入口 |
| --- | --- | --- |
| `choice` | prepared Offer handoff、pending Interruption、waiting Activity、`awaiting_choice` Run | 用户提交已校验的 ChoiceDecision |
| `approval` | prepared approval handoff、pending Interruption、冻结的 SDK RunState、waiting Activity、`awaiting_approval` Run | 用户批准或拒绝同一 approval identity |
| `task` | prepared Task handoff、Task batch、Run Wait、waiting Activity、`awaiting_task` Run | Outbox 的唯一 continuation command |
...
| Choice / Approval 的 pending Interaction | `execution-handoff`；锁 Run、准备 intent 后同事务写 message、替换旧 pending、写 Event/Activity/Interruption/Outbox | runtime、Session、control route |
...
| 协议结算回执 | Choice 为当前 execution fence 的内存 scoped handoff receipt；最终 Interaction receipt 只能在上述事务成功提交后登记 | invocation、tool adapter、stop controller |
| assistant prose | execution-handoff 的同一事务；与可恢复 Interaction/Task 交接共同提交 | Thread UI |

Choice/Approval 卡片已经包含继续所需的完整 Offer；Task Wait 已经包含继续所需的 Task identity。它们仍是独立领域事实，但一次执行段把用户可见 prose 与这些事实交接给用户时，必须在同一可恢复事务提交；否则刷新会看到卡片或文本的半边。Choice、Approval、Task 都先保存不可见 handoff，随后由唯一 settlement 同时提交 message、Interaction/Wait、Activity、Run 与 Outbox。真实进程崩溃发生在 prepare 后会从 handoff 恢复，而不会丢失可继续状态。
...
| Choice settlement | `prepare/settleProjectAgentPreparedChoiceHandoff` | Choice 专属 post-fence 和 `choiceExecutionOutcome` |
| Approval settlement | `prepare/settleProjectAgentPreparedApprovalHandoff` | runtime 直接创建 approval 的旁路 |
| Task settlement | Task batch transaction prepare + `settleProjectAgentPreparedTaskHandoff` | runtime 在 Task commit 后补建 Wait；只按工具输出猜测 batch |
...
修改前后数量：Choice、Approval、Task 由“各自写卡片/Wait + 后置 verifier/finalizer”收敛为“prepare intent + execution-handoff settlement”两步；Run status 不再参与 execution postcondition。runtime 的停止结论不再有按业务 Choice identity 的分支。
...
本阶段引入 `ProjectAgentExecutionHandoff` migration；它是唯一可恢复的、不可见 execution-segment intent，不是第二套 UI 状态机。
...
替换为：协议级 `suspendsFor` 声明、Choice handoff receipt、`execution-handoff` 唯一 settlement 与通用 verifier。没有双轨兼容期；所有已注册 Choice、Approval 与 Task 同时迁移。
