...
Make the self-hosted project video-model configuration accept only runtime-selectable local ComfyUI video models through one authoritative policy. The current selectable model is `comfyui::minimax-h3-fast`. Tool callers, MCP-published Tool callers, and API callers must receive the same decision before `Project.videoModel` is persisted.

## Current Facts

- `update_project_config` is published to Tool/MCP callers and also exposes a canonical API channel.
- The Tool canonicalizer validates a requested video model through `resolveModelSelection`, while the canonical API input can reach the transaction writer with only syntactic `provider::modelId` validation.
- `update_user_preference` and the full API-config save path can persist an unvalidated video-model default, and `create_project` copies that value into a new project without resolving it.
...
- Apply the same validation to self-hosted user video preferences and to the project-creation inheritance path, so no persistent writer can introduce a non-local project video model.
- Preserve `null` as the explicit command for clearing the project video-model override through canonical API input.
...
Both runtime invocation channels converge on `executeInTransaction`: direct API calls use the `api` channel, while Agent and MCP-published tools use the `tool` channel. Before constructing the project update:
...
The Tool canonicalizer only maps the Agent command into canonical operation input. It does not own the final business decision. This leaves one authoritative model resolver for Tool, API, and MCP-published Tool calls.

Both user-preference writers resolve a non-null self-hosted video default before their upsert. `create_project` resolves any inherited self-hosted video default before inserting the project. Existing invalid preference data therefore fails explicitly instead of being copied into a new project.
...
The rejection happens before persistence. The existing project or user preference remains unchanged, and project creation does not insert a partially configured project.
...
- A wrong-modality model and an unknown model are rejected by the shared writer after either Tool/MCP command canonicalization or canonical API parsing.
- Rejected values are not persisted.
- An unavailable self-hosted video preference cannot be stored or inherited into a new project.
