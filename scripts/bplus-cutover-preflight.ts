...
function supportsRequiredMysqlVersion(version: readonly [number, number, number]): boolean {
  const [major, minor, patch] = version
  return major > 8 || (major === 8 && (minor > 0 || (minor === 0 && patch >= 17)))
}

async function queryRows(prisma: PrismaClient, sql: string): Promise<QueryRow[]> {
...
function requireStringField(row: QueryRow, field: string, errorCode: string): string {
...
async function readCharacterColumnDefinition(
  ...
  const row = rows[0]
  if (!row) {
    throw new Error(`BPLUS_PREFLIGHT_CHARACTER_COLUMN_MISSING:${tableName}.${columnName}`)
  }
...
async function assertExpectedLegacyShape(prisma: PrismaClient): Promise<void> {
  ...
  if (missingTables.length > 0) {
    throw new Error(`BPLUS_PREFLIGHT_LEGACY_TABLES_MISSING:${missingTables.join(',')}`)
  }
  ...
  if (partialTables.length > 0) {
    throw new Error(`BPLUS_PREFLIGHT_PARTIAL_CUTOVER_TABLES_PRESENT:${partialTables.join(',')}`)
  }

  const missingColumns: string[] = []
  for (const [tableName, columns] of Object.entries(REQUIRED_LEGACY_COLUMNS)) {
  ...
  if (missingColumns.length > 0) {
    throw new Error(`BPLUS_PREFLIGHT_LEGACY_COLUMNS_MISSING:${missingColumns.join(',')}`)
  }

  const partialColumns: string[] = []
  for (const [tableName, columns] of Object.entries(FORBIDDEN_PARTIAL_CUTOVER_COLUMNS)) {
    for (const columnName of columns) {
      if (await columnExists(prisma, tableName, columnName)) {
  ...
  if (partialColumns.length > 0) {
    throw new Error(`BPLUS_PREFLIGHT_PARTIAL_CUTOVER_COLUMNS_PRESENT:${partialColumns.join(',')}`)
  }
}

async function assertArchiveShape(prisma: PrismaClient): Promise<void> {
  const userIdDefinition = await readCharacterColumnDefinition(prisma, 'user', 'id')
  ...
  for (const columnName of ARCHIVE_COLUMNS) {
    if (!(await columnExists(prisma, 'project_assistant_thread_archives', columnName))) {
      missingColumns.push(columnName)
    }
  }
  if (missingColumns.length > 0) {
    throw new Error(`BPLUS_PREFLIGHT_ARCHIVE_COLUMNS_MISSING:${missingColumns.join(',')}`)
  }
  assertArchiveUserIdForeignKeyCompatibility({
  ...
  const orphanUsers = await queryRows(
    prisma,
    `
...
async function run(): Promise<void> {
  ...
  try {
    ...
    if (!supportsRequiredMysqlVersion(version)) {
      throw new Error(`BPLUS_PREFLIGHT_MYSQL_VERSION_UNSUPPORTED:${version.join('.')}`)
    }
    ...
    if (blockerTotal > 0) {
      throw new Error(`BPLUS_PREFLIGHT_BLOCKED:${String(blockerTotal)}`)
    }
    console.log(
      stringifyForLog({
