...
const criticalLocalizedTemplateTokens = new Map([
  ['project-agent/system', {
    en: [
      ...
      'submits one or more background `CREATIVE_WORK` Tasks',
      '`kind=single` submits one request',
      '`kind=batch` submits every independent request as its own Task',
      '`delegate_creative_work(kind=chapter_batch)`',
      ...
      'Every video production must first delegate `style_bible` with `design.mode=final`',
      'call `adopt_style_bible`',
    ...
    zh: [
      ...
      '提交一个或多个后台 `CREATIVE_WORK` Task',
      '`kind=single` 提交一个请求',
      '`kind=batch` 把每个独立请求分别提交为一个 Task',
      '`delegate_creative_work(kind=chapter_batch)`',
      'Task 全部终态后的续跑只返回完成摘要。',
      '用该回执中的精确 `taskId` 调用 `get_task` 读取',
      'Bible Task 完成只会恢复主 Agent',
      '任何视频制作都必须先委派 `style_bible` 得到 `design.mode=final`',
      '调用 `adopt_style_bible`',
...
const criticalCreativeSkillTokens = new Map([
  ...
  ['video-direction', {
    en: ['# Video Direction and Generation Design', 'finalized Style Bible with exact provenance', 'explicitly ordered reference manifest', '“cut to the location in image N,”', 'one to three core actions', 'ignore caller prose that prescribes', 'do not split material that naturally fits one 15-second generation', 'never divide one unfinished action across two generations', '`{spoken line}`', '`<sound description>`', 'objects crossing the lens', 'otherwise tell the user plainly', '## Dialogue, sound, and native audio'],
    zh: ['# 视频导演与生成设计', '已经确认并具有精确来源的最终 Style Bible', '有明确顺序的参考清单', '“镜头切至图片N中的……”', '一到三个核心动作', '忽略调用方文字里预设的', '能自然承载 15 秒的内容就不要拆成多个更短片段', '不把一个未完成动作切到两次独立生成', '`{逐字台词}`', '`<声音描述>`', '物体擦过镜头', '没有该能力时必须明确告诉用户', '## 对白、声音和原生音频'],
  }],
  ['music-direction', {
