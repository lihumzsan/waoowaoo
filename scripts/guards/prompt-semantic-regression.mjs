...
const criticalLocalizedTemplateTokens = new Map([
  ['project-agent/system', {
    en: [
      ...
      'Delegate `style_bible` for the former and `asset_prompt_set` for the latter',
      'pass its `generationPrompt` directly to the real image Operation',
      'delegate `edit_bible_bundle`',
      '`save_edit_source`',
      '`adopt_edit_bible_bundle`',
      '# Short tasks and long works',
      'For a roughly ten-second clip',
      'one Chapter per Subagent Task',
      'both the structured director timeline and directly executable final video-model prompt',
    ...
    zh: [
      ...
      '需要前者时委派 `style_bible`，需要后者时委派 `asset_prompt_set`',
      '把 `generationPrompt` 直接交给真实图片 Operation',
      '委派 `edit_bible_bundle`',
      '`save_edit_source`',
      '`adopt_edit_bible_bundle`',
      '# 短任务与长作品',
      '用户只要一支可由单次生成完成的约十秒短片时',
      '每个 Chapter 对应一个 Subagent Task',
      '同时包含结构化导演时间线与可直接提交的视频模型最终 prompt',
