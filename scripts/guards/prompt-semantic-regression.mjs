...
const criticalCreativeSkillTokens = new Map([
  ...
  ['video-direction', {
    en: ['# Video Direction and Generation Design', 'finalized Style Bible with exact provenance', 'explicitly ordered reference manifest', 'one final video prompt', '“cut to the location in image N”', 'one to three core actions', 'ignore caller prose that prescribes', 'do not split material that naturally fits one 15-second generation', 'never divide one unfinished action across two generations', '`{spoken line}`', '`<sound description>`', '**Dark/black bridge**', '**Montage transition**', '**Metaphorical transition**', '**Creative transition**', 'Do not use one at every scene boundary', '## Excellent complete prompt examples', '### Example one: one scene without a creative transition', '### Example three: a motivated dark and metaphorical transition', 'Native audio is enabled by default', '## Dialogue, sound, and native audio'],
    zh: ['# 视频导演与生成设计', '已经确认并具有精确来源的最终 Style Bible', '有明确顺序的参考清单', '唯一一份最终视频提示词', '“镜头切至图片N中的……”', '一到三个核心动作', '忽略调用方文字里预设的', '能自然承载 15 秒的内容就不要拆成多个更短片段', '不把一个未完成动作切到两次独立生成', '`{逐字台词}`', '`<声音描述>`', '**黑暗/黑场衔接**', '**蒙太奇转场**', '**隐喻转场**', '**创意转场**', '不是每个场景都用', '## 优秀完整提示词示例', '### 示例一：同一场景，不使用创意转场', '### 示例三：有动机的黑暗与隐喻转场', '视频生成默认开启原生音频', '## 对白、声音和原生音频'],
  }],
  ['music-direction', {
